...
void createFan(void) { engineFan_init(getFanInstance()); }

engineFan *getFanInstance(void) { return &getECUContext()->fan; }
...
void engineFan_showDebug(const engineFan *self) {
  deb("fan enabled: %d reason: %d", engineFan_isFanEnabled(self),
      engineFan_fanEnabledReason(self));
}
