...
void test_turbo_uses_last_rpm_row_when_rpm_exceeds_limit(void) {
  ...
  setGlobalValue(F_INTAKE_TEMP,
                 (float)(MIN_TEMPERATURE_CORRECTION - 1)); // no temp correction

  Turbo_process(&turbo);

  const int expectedPressure =
      RPM_table[RPM_PRESCALERS - 1][N75_PERCENT_VALS - 1];
...
void test_turbo_overboost_path_never_sets_negative_pressure_percentage(void) {
  ...
  setGlobalValue(F_PRESSURE,
                 (float)(MAX_BOOST_PRESSURE + 0.2f)); // overboost branch
