...
static void hwBuildFrame(uint16_t seq, const char *payload, char *out,
                         size_t out_size) {
  char body[160];
  const int body_len =
...
static void testSoftTimerCallback(void) { s_soft_timer_hits++; }
...
void test_pid_stability_and_oscillation_helpers(void) {
  ...
  (void)hal_pid_controller_is_oscillating(pid, 1.0f, 6);
  (void)hal_pid_controller_is_oscillating(pid, -1.0f, 6);
  (void)hal_pid_controller_is_oscillating(pid, 1.0f, 6);
  (void)hal_pid_controller_is_oscillating(pid, -1.0f, 6);
  (void)hal_pid_controller_is_oscillating(pid, 1.0f, 6);
  const bool oscillating = hal_pid_controller_is_oscillating(pid, -1.0f, 6);
...
void test_pid_null_safety(void) {
  ...
  TEST_ASSERT_FLOAT_WITHIN(0.0001f, 0.0f,
                           hal_pid_controller_update(NULL, 1.0f));
