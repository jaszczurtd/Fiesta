...
void test_ford_part_split_standard(void) {
  ...
  int pLen, mLen, sLen;
  TEST_ASSERT_TRUE(fordPartNumberSplit("YS6F-12A650-FA", &prefix, &pLen,
                                       &middle, &mLen, &suffix, &sLen));
...
void test_ford_part_split_three_char_suffix(void) {
  const char *prefix, *middle, *suffix;
  int pLen, mLen, sLen;
  TEST_ASSERT_TRUE(fordPartNumberSplit("XS4A-12A650-AXB", &prefix, &pLen,
                                       &middle, &mLen, &suffix, &sLen));
  TEST_ASSERT_EQUAL_INT(4, pLen);
  TEST_ASSERT_EQUAL_INT(6, mLen);
...
void test_ford_part_split_no_dashes_fails(void) {
  const char *prefix, *middle, *suffix;
  int pLen, mLen, sLen;
  TEST_ASSERT_FALSE(fordPartNumberSplit("NODASHES", &prefix, &pLen, &middle,
                                        &mLen, &suffix, &sLen));
}
...
void test_ford_part_split_one_dash_fails(void) {
  const char *prefix, *middle, *suffix;
  int pLen, mLen, sLen;
  TEST_ASSERT_FALSE(fordPartNumberSplit("PREFIX-REST", &prefix, &pLen, &middle,
                                        &mLen, &suffix, &sLen));
}
...
void test_dtc_timestamp_first_occurrence_from_gps(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(expected,
                           dtcManagerGetTimestamp(DTC_OBD_CAN_INIT_FAIL));
}
...
void test_dtc_timestamp_not_overwritten_on_reactivation(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(firstTs,
                           dtcManagerGetTimestamp(DTC_PCF8574_COMM_FAIL));
}
...
void test_uds_diag_session_extended_positive_response(void) {
  uint8_t req[8] = {
      0x02, UDS_SVC_DIAGNOSTIC_SESSION, UDS_SESSION_EXTENDED, 0, 0, 0, 0, 0};
...
void test_uds_tester_present_suppress_positive_response_sends_nothing(void) {
  uint8_t req[8] = {
      0x02, UDS_SVC_TESTER_PRESENT, UDS_SUPPRESS_POSITIVE_RSP, 0, 0, 0, 0, 0};
  obdReq(LISTEN_ID, req);
...
void test_uds_read_active_session_did_reflects_last_session(void) {
  ...
  uint8_t didReq[8] = {0x03,
                       UDS_SVC_READ_DATA_BY_ID,
                       MSB(DID_ACTIVE_SESSION),
                       LSB(DID_ACTIVE_SESSION),
                       0,
                       0,
                       0,
                       0};
...
void test_uds_read_memory_by_addr_unmapped_returns_zero_filled_block(void) {
  uint8_t req[8] = {0x04, UDS_SVC_READ_MEMORY_BY_ADDR, 0x00, 0x12, 0x34, 0, 0,
                    0};
  obdReq(LISTEN_ID, req);
...
void test_uds_read_memory_by_addr_invalid_type_returns_general_negative(void) {
  uint8_t req[8] = {0x04, UDS_SVC_READ_MEMORY_BY_ADDR, 0x7F, 0x12, 0x34, 0, 0,
                    0};
  obdReq(LISTEN_ID, req);
...
void test_uds_read_ecu_capabilities_returns_nrc31(void) {
  uint8_t req[8] = {0x03,
                    UDS_SVC_READ_DATA_BY_ID,
                    MSB(DID_ECU_CAPABILITIES),
                    LSB(DID_ECU_CAPABILITIES),
                    0,
                    0,
                    0,
                    0};
...
void test_uds_read_scp_bp_returns_default_barometric_value(void) {
  uint8_t req[8] = {
      0x03, UDS_SVC_READ_DATA_BY_ID, MSB(SCP_PID_BP), LSB(SCP_PID_BP), 0, 0, 0,
      0};
  obdReq(LISTEN_ID, req);
...
void test_uds_read_scp_idblock_addr_returns_descriptor(void) {
  uint8_t req[8] = {0x03,
                    UDS_SVC_READ_DATA_BY_ID,
                    MSB(SCP_PID_IDBLOCK_ADDR),
                    LSB(SCP_PID_IDBLOCK_ADDR),
                    0,
                    0,
                    0,
                    0};
...
void test_uds_read_f4_catch_code_alt_prefers_live_pid_payload(void) {
  ...
  uint8_t req[8] = {0x03,
                    UDS_SVC_READ_DATA_BY_ID,
                    MSB(DID_F4_CATCH_CODE_ALT),
                    LSB(DID_F4_CATCH_CODE_ALT),
                    0,
                    0,
                    0,
                    0};
...
void test_kwp_read_local_id_rom_size_returns_512k(void) {
  uint8_t req[8] = {
      0x02, UDS_SVC_READ_DATA_BY_LOCAL_ID, KWP_LID_ROM_SIZE, 0, 0, 0, 0, 0};
  obdReq(LISTEN_ID, req);
...
void test_kwp_read_local_id_compact_ident_returns_nrc31(void) {
  uint8_t req[8] = {
      0x02, UDS_SVC_READ_DATA_BY_LOCAL_ID, KWP_LID_COMPACT_IDENT, 0, 0, 0, 0,
      0};
  obdReq(LISTEN_ID, req);
...
void test_uds_read_data_by_id_short_frame_returns_nrc13(void) {
  ...
  // numofBytes=2 supplies only one DID byte and must trigger
...
void test_uds_read_memory_by_addr_short_frame_returns_nrc13(void) {
  // UDS_SVC_READ_MEMORY_BY_ADDR requires minLen=4 (service + DMR type + 2-byte
  // addr). numofBytes=3 is one byte short and must trigger
  // NRC_INCORRECT_LENGTH.
...
int main(void) {
  ...
  RUN_TEST(
      test_uds_read_dtc_info_returns_service_not_supported_in_fordiag_mode);
