...
static void performAuth(void) {
  ...
  TEST_ASSERT_TRUE(hal_sc_auth_compute_response(
      key, challenge, sizeof(challenge), configSessionId(), mac));
...
void test_sc_set_param_rejects_without_auth(void) {
  ...
  /* No AUTH_PROVE. */
  const char *response =
      sendSerialLine("SC_SET_PARAM fan_coolant_start_c 110\n");
...
void test_sc_set_param_rejects_out_of_range(void) {
  ...
  const char *response =
      sendSerialLine("SC_SET_PARAM fan_coolant_start_c 200\n");
  TEST_ASSERT_NOT_NULL(strstr(response, "SC_BAD_REQUEST"));
...
void test_sc_set_param_rejects_malformed_payload(void) {
  ...
  const char *missingValue =
      sendSerialLine("SC_SET_PARAM fan_coolant_start_c\n");
  TEST_ASSERT_NOT_NULL(strstr(missingValue, "SC_BAD_REQUEST"));

  /* Non-numeric value. */
  const char *badValue =
      sendSerialLine("SC_SET_PARAM fan_coolant_start_c xyz\n");
  TEST_ASSERT_NOT_NULL(strstr(badValue, "SC_BAD_REQUEST"));
  TEST_ASSERT_NOT_NULL(strstr(badValue, "value_not_int16"));

  /* Out of int16 domain. */
  const char *outOfDomain =
      sendSerialLine("SC_SET_PARAM fan_coolant_start_c 70000\n");
