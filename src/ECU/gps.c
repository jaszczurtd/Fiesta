...
static gps_runtime_state_t s_gpsRuntime = {.isGPSInitialized = false};
...
void getGPSData(void) {
  ...
  if (gpsAvailable) {
    ...
    setGlobalValue(F_GPS_DATE,
                   (float)(((year % 100) * 10000) + (month * 100) + day));
  } else {
  ...
  if (hal_gps_location_is_valid()) {
    if (hal_gps_location_is_updated()) {
      ...
      deb("Lat=%f Long=%f date:%s hour:%s", hal_gps_latitude(),
          hal_gps_longitude(), getGPSDate(), getGPSTime());
    }
  } else {
    deb("GPS is not available");
  }

  deb("GPS: valid:%d updated:%d age:%lu", hal_gps_location_is_valid(),
      hal_gps_location_is_updated(), (unsigned long)hal_gps_location_age());
}
...
const char *getGPSDate(void) {
  if (hal_gps_location_is_valid()) {
    ...
    getAdjustedDateTime(&year, &month, &day, &hour, &minute, &second);
    memset(s_gpsPersistent.gpsDate, 0, GPS_TIME_DATE_BUFFER_SIZE);
    snprintf(s_gpsPersistent.gpsDate, GPS_TIME_DATE_BUFFER_SIZE - 1,
...
const char *getGPSTime(void) {
  if (hal_gps_location_is_valid()) {
    ...
    getAdjustedDateTime(&year, &month, &day, &hour, &minute, &second);
    memset(s_gpsPersistent.gpsTime, 0, GPS_TIME_DATE_BUFFER_SIZE);
    snprintf(s_gpsPersistent.gpsTime, GPS_TIME_DATE_BUFFER_SIZE - 1,
