...
void VP37_showDebug(VP37Pump *self) {
  ...
  if (hal_millis_interval_elapsed_now(&lastTelemetryMs,
    ...
    deb("VP37 CFG rev:83 kp:%.4f ki:%.4f kd:%.5f topkd:%.5f dkeff:%.5f "
        "tf:%.4f tu:%.1f "
        ...
        (self->thermal.driveVoltageSettled && self->thermal.driveOnsetPassed)
            ? 0U
            : 1U,
        ...
        (unsigned long)self->currentControl.sampleAgeUs, self->pid.effectiveKp,
        self->feedback.leadWeight);
...
static VP37TraceSample VP37_controlSample(const VP37Pump *self) {
  const VP37TraceSample sample = {
      ...
      .feedbackLead = self->pid.feedbackLead,
...
static void VP37_showControlSample(const VP37TraceSample *sample,
  ...
  deb("VP37 %s us:%lu dt:%lu n:%lu thr:%.1f tar:%d des:%d adj:%d pwm:%d err:%d "
      ...
      "ft:%.0f tcf:%.4f tsc:%.4f mff:%.1f cyms:%lu mtrim:%.1f lead:%.1f",
