...
void VP37_showDebug(VP37Pump *self) {
  ...
  if (hal_millis_interval_elapsed_now(&lastTelemetryMs,
    ...
    deb("VP37 CFG rev:75 kp:%.4f ki:%.4f kd:%.5f tf:%.4f tu:%.1f "
        ...
        self->pid.pidKp, self->pid.pidKi, self->pid.pidKd, self->pid.pidTf,
        self->pidTimeUpdate, self->feedback.VP37_ADJUST_MIN,
        self->feedback.VP37_ADJUST_MAX, self->supply.lastVolts,
        self->supply.localVolts, self->supply.compensationInputVolts,
        self->supply.compensationVolts, self->supply.localVoltageScale,
        VP37_VOLTAGE_FILTER_S, self->supply.voltageCorrection,
        self->thermal.lastFuelTemp, self->pid.pidIntegralLimit,
        ...
        self->supply.cycleVoltageUsed ? 1U : 0U,
        self->supply.voltageFrozen ? 1U : 0U, self->supply.cycleSupplyVolts,
        (unsigned)VP37_PWM_FREQUENCY_HZ, self->thermal.cycleCurrentAmps,
        ...
        self->feedforward.motionBoostDown, self->scan.scanRunning ? 1U : 0U,
        (unsigned long)self->scan.scanFrameNs,
        (unsigned long)self->scan.scanBlocks,
        (unsigned long)self->scan.scanGaps);
...
void VP37_showCurrentPulse(const VP37Pump *self) {
  ...
  deb("VP37 IPULSE us:%lu seq:%lu state_us:%lu pwm:%ld adj:%ld des:%ld "
      ...
      (long)self->feedback.currentAdjustometerPosition,
      (long)self->demand.desiredAdjustometer, self->supply.compensationVolts,
      self->thermal.lastFuelTemp, result->meanAmps, result->p95Amps,
      result->peakAmps, (unsigned long)result->periodUs,
      ...
      result->supplyValid ? 1U : 0U, (unsigned long)self->scan.scanBlocks,
      (unsigned long)self->scan.scanGaps, (unsigned long)result->glitches);
}
...
static VP37TraceSample VP37_controlSample(const VP37Pump *self) {
  const VP37TraceSample sample = {
      ...
      .target = self->demand.desiredAdjustometerTarget,
      .desired = self->demand.desiredAdjustometer,
      .measured = self->feedback.currentAdjustometerPosition,
      .pwm = self->output.finalPWM,
      .ff = self->feedforward.pwmFeedForward,
      .low = self->pid.pidNegativeLimit,
      .motionFF = self->feedforward.feedForwardMotion,
      .high = self->pid.pidUpperLimit,
      .volts = self->supply.lastVolts,
      .localVolts = self->supply.localVolts,
      .compensationInputVolts = self->supply.compensationInputVolts,
      .compensationVolts = self->supply.compensationVolts,
      .voltageCorrection = self->supply.voltageCorrection,
      .cycleVoltageUsed = self->supply.cycleVoltageUsed,
      .voltageOverRange = self->supply.voltageOverRange,
      .mapTrim = self->feedforward.mapTrimApplied,
      .thermalScale = self->thermal.thermalScale,
      ...
      .terms = self->pid.pidTerms,
      .softFloor = self->pid.softFloorActive,
      .hardwareClamp = self->output.pwmLimited,
      .quantityAtRest = self->demand.quantityAtRest,
      .status = self->feedback.lastAdjustometerStatus,
      .rawHz = self->feedback.feedbackRawHz,
      .filteredHz = self->feedback.feedbackFilteredHz,
      .sampleNumber = self->feedback.feedbackNumber,
      .measuredUs = self->feedback.feedbackUs,
      .ageUs = self->feedback.feedbackAgeUs,
      .readStatus = self->feedback.feedbackReadStatus,
      .readUs = self->feedback.feedbackReadUs,
      .retries = self->feedback.feedbackRetries,
      .fresh = self->feedback.feedbackFresh,
