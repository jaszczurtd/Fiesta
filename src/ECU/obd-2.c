...
static bool requireMinLength(uint32_t responseId, uint8_t serviceId,
                             uint8_t numofBytes, uint8_t minLen);
TESTABLE_STATIC uint8_t stMinToMs(uint8_t stMin);
static bool handleMode01(uint8_t pid, uint32_t responseId, uint8_t mode,
                         uint8_t *txData, bool *tx);
static bool handleMode06(uint8_t pid, uint32_t responseId, uint8_t mode,
                         uint8_t *txData, bool *tx);
static bool handleMode09(uint8_t pid, uint32_t responseId, uint8_t mode,
                         uint8_t *txData, bool *tx);
static bool handleObdService(uint8_t mode, uint8_t pid, uint32_t responseId,
                             uint8_t *txData, bool *tx);
static bool handleUdsService(uint8_t mode, uint8_t numofBytes,
                             const uint8_t *data, uint32_t responseId,
                             uint8_t *txData, bool *tx);
static void handleUdsReadDtcInfo(uint8_t mode, uint8_t numofBytes,
                                 const uint8_t *data, uint32_t responseId,
                                 uint8_t *txData, bool *tx);
static void handleUdsReadDataById(uint8_t mode, uint8_t numofBytes,
                                  const uint8_t *data, uint32_t responseId,
                                  uint8_t *txData, bool *tx);
static void handleUdsReadDataByLocalId(uint8_t mode, uint8_t numofBytes,
                                       const uint8_t *data,
                                       uint32_t responseId);
static bool handleScpPidAccess(uint32_t responseId, uint16_t pid,
                               uint8_t *txData, bool *tx);
static void sendScpGeneralResponse(uint32_t responseId, uint8_t requestMode,
                                   uint8_t arg1, uint8_t arg2, uint8_t arg3,
                                   uint8_t responseCode);
static void sendScpDmrResponse(uint32_t responseId, uint16_t addr,
                               uint8_t dmrType);
...
static bool handleMode09(uint8_t pid, uint32_t responseId, uint8_t mode,
  ...
  if (pid == 0x00u) { // Supported PIDs 01-20
    ...
    *tx = true;
    ...
    uint8_t CID[3 + 16] = {(uint8_t)(UDS_POSITIVE_RESPONSE_OFFSET | mode), pid,
                           0x01};
    ...
    *tx = true;
    ...
    uint8_t ECMname[3 + 20] = {(uint8_t)(UDS_POSITIVE_RESPONSE_OFFSET | mode),
                               pid, 0x01};
    ...
    uint8_t typeAppr[3 + 20] = {(uint8_t)(UDS_POSITIVE_RESPONSE_OFFSET | mode),
                                pid, 0x01};
...
static bool handleObdService(uint8_t mode, uint8_t pid, uint32_t responseId,
  ...
  if (mode == (uint8_t)OBD_MODE_CURRENT_DATA) {
    handled = handleMode01(pid, responseId, mode, txData, tx);
    ...
    *tx = true;
    ...
    handled = handleMode09(pid, responseId, mode, txData, tx);
 ...
 * @return None.
 */
static void send22Field(uint32_t responseId, uint16_t did, const char *str,
                        int width) {
  if (width < 0)
    width = 0;
  if (width > 40)
  ...
  uint8_t payload[3 + 40] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                             (uint8_t)(did & 0xFF)};
 ...
 * @return None.
 */
static void send22IdentField(uint32_t responseId, uint16_t did, const char *str,
                             int width) {
  if (width < 0)
    width = 0;
  if (width > 40)
...
static void send22U32(uint32_t responseId, uint16_t did, uint32_t value) {
  uint8_t payload[7] = {UDS_RSP_READ_DATA_BY_ID,
                        (uint8_t)(did >> 8),
                        (uint8_t)(did & 0xFF),
                        0,
                        0,
...
static void sendE217PartNumMiddle(uint32_t responseId, uint16_t did) {
  ...
  if (midLen > 8)
    midLen = 8;
  uint8_t payload[3 + 8] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                            (uint8_t)(did & 0xFF)};
...
static void sendE21APartNumPrefix(uint32_t responseId, uint16_t did) {
  ...
  if (prefixLen > 16)
    prefixLen = 16;
  uint8_t payload[3 + 16] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                             (uint8_t)(did & 0xFF)};
...
static void sendE219PartNumSuffix(uint32_t responseId, uint16_t did) {
  ...
  uint8_t payload[5] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                        (uint8_t)(did & 0xFF), leftByte, rightByte};
#ifdef OBD_VERBOSE_IDENT_DEBUG
 ...
 * @return None.
 */
static void send12LocalField(uint32_t responseId, uint8_t localId,
...
static void sendScpGeneralResponse(uint32_t responseId, uint8_t requestMode,
                                   uint8_t arg1, uint8_t arg2, uint8_t arg3,
                                   uint8_t responseCode) {
  uint8_t rsp[8] = {0x06, UDS_RSP_NEGATIVE, requestMode, arg1, arg2,
...
static void sendScpDmrResponse(uint32_t responseId, uint16_t addr,
  ...
  readScpDmrByte(dmrType, (uint16_t)(addr + 1u), &b1);
  readScpDmrByte(dmrType, (uint16_t)(addr + 2u), &b2);
  readScpDmrByte(dmrType, (uint16_t)(addr + 3u), &b3);

  uint8_t rsp[8] = {
      0x07, UDS_RSP_READ_MEMORY_BY_ADDR, MSB(addr), LSB(addr), b0, b1, b2, b3};
#ifdef OBD_VERBOSE_IDENT_DEBUG
...
static void handleUdsReadDataById(uint8_t mode, uint8_t numofBytes,
  ...
  if (isFordDiagIdentificationDid(did)) {
    deb("UDS 0x22 ident request DID=0x%04X reqId=0x%03lX", did,
        (unsigned long)s_obdState.activeRequestIdValue);
    hal_deb_hex("UDS 0x22 ident request raw", data, (int)numofBytes + 1, 16);
  }
#endif
  if (did == (uint16_t)DID_VIN) {
    uint8_t payload[] = {UDS_RSP_READ_DATA_BY_ID,
                         (uint8_t)(DID_VIN >> 8),
                         (uint8_t)(DID_VIN & 0xFF),
                         PAD,
                         PAD,
                         PAD,
                         PAD,
    ...
    uint8_t udsRsp[] = {0x04,
                        UDS_RSP_READ_DATA_BY_ID,
                        (uint8_t)(DID_ACTIVE_SESSION >> 8),
                        (uint8_t)(DID_ACTIVE_SESSION & 0xFF),
                        s_obdState.udsSessionValue,
                        PAD,
                        PAD,
                        PAD};
    ...
    if (encodeMode01PidData(pid, dataBytes, &dataLen)) {
      uint8_t payload[3 + 4] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                                (uint8_t)did};
      ...
      uint8_t payload[] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                           (uint8_t)did, 0x00};
      iso_tp(responseId, (int)sizeof(payload), payload);
    }
    ...
    uint8_t payload[6] = {
        ...
        (uint8_t)(month & 0x0F), (uint8_t)day,        (uint8_t)(year - 1900)};
    ...
    uint8_t payload[6] = {
        UDS_RSP_READ_DATA_BY_ID,     (uint8_t)(did >> 8),
        (uint8_t)(did & 0xFF),       (uint8_t)((km >> 16) & 0xFF),
        (uint8_t)((km >> 8) & 0xFF), (uint8_t)(km & 0xFF)};
    ...
    uint8_t payload[4] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                          (uint8_t)(did & 0xFF), raw};
    ...
    uint8_t payload[5] = {UDS_RSP_READ_DATA_BY_ID, (uint8_t)(did >> 8),
                          (uint8_t)(did & 0xFF), (uint8_t)(raw >> 8),
                          (uint8_t)(raw & 0xFF)};
    iso_tp(responseId, (int)sizeof(payload), payload);
...
static void handleUdsReadDataByLocalId(uint8_t mode, uint8_t numofBytes,
  ...
  case KWP_LID_CALIBRATION_ID:
    send12LocalField(responseId, localId, ecu_CalibrationId, 16);
    break;
  case KWP_LID_SW_DATE:
    send12LocalField(responseId, localId, ecu_SwDate, 8);
    break;
  case KWP_LID_PART_NUMBER:
    send12LocalField(responseId, localId, ecu_PartNumber, 16);
    break;
  case KWP_LID_MODEL_16:
    send12LocalField(responseId, localId, ecu_Model, 16);
    break;
  case KWP_LID_VIN:
    send12LocalField(responseId, localId, vehicle_Vin, 17);
    break;
  case KWP_LID_MODEL:
    send12LocalField(responseId, localId, ecu_Model, 16);
    break;
  case KWP_LID_TYPE:
    send12LocalField(responseId, localId, ecu_Type, 8);
    break;
  case KWP_LID_SUBTYPE:
    send12LocalField(responseId, localId, ecu_SubType, 8);
    break;
  case KWP_LID_CATCH_CODE:
    send12LocalField(responseId, localId, ecu_CatchCode, 8);
    break;
  case KWP_LID_VIN_ALT:
    send12LocalField(responseId, localId, vehicle_Vin, 17);
    break;
  case KWP_LID_SW_VERSION:
    send12LocalField(responseId, localId, ecu_SwVersion, 4);
    break;
  ...
  case KWP_LID_HARDWARE_ID:
    send12LocalField(responseId, localId, ecu_HardwareId, 8);
    break;
  case KWP_LID_COPYRIGHT:
    send12LocalField(responseId, localId, ecu_Copyright, 32);
    break;
  ...
  if (mode == (uint8_t)UDS_SVC_DIAGNOSTIC_SESSION) {
    ...
    if (requireMinLength(responseId, mode, numofBytes, 2)) {
      ...
      if ((subFunction == (uint8_t)UDS_SESSION_DEFAULT) ||
        ...
        uint8_t udsRsp[] = {0x06,        UDS_RSP_DIAGNOSTIC_SESSION,
                            subFunction, 0x00,
                            0x32,        0x01,
                            0xF4,        PAD};
        hal_can_send(s_obdState.canHandle, responseId, 8, udsRsp);
    ...
    *tx = true;
    ...
    handled = true;
    handleUdsReadDtcInfo(mode, numofBytes, data, responseId, txData, tx);
    ...
    if (requireMinLength(responseId, mode, numofBytes, 4)) {
      ...
      if (!validType) {
        sendScpGeneralResponse(responseId, UDS_SVC_READ_MEMORY_BY_ADDR, data[2],
    ...
    if (requireMinLength(responseId, mode, numofBytes, 3)) {
      handleUdsReadDataById(mode, numofBytes, data, responseId, txData, tx);
    }
  } else if (mode == (uint8_t)UDS_SVC_READ_DATA_BY_LOCAL_ID) {
    handled = true;
    // KWP2000 service 0x12 - ReadDataByLocalIdentifier
    if (requireMinLength(responseId, mode, numofBytes, 2)) {
      handleUdsReadDataByLocalId(mode, numofBytes, data, responseId);
    }
...
void obdReqWithDlc(uint32_t requestId, uint8_t dlc, const uint8_t *data) {
  ...
  bool tx = false;
  uint8_t txData[] = {0x00, (uint8_t)(UDS_POSITIVE_RESPONSE_OFFSET | mode),
                      pid,  PAD,
                      PAD,  PAD,
                      PAD,  PAD};

  if (mode == OBD_MODE_CURRENT_DATA && pid <= PID_LAST) {
    deb("OBD-2 pid:0x%02x (%s) length:0x%02x mode:0x%02x", pid, getPIDName(pid),
        numofBytes, mode);
  } else {
...
static void iso_tp_process(void) {
  ...
  if (s_obdState.isoTpState.state == ISO_TP_WAIT_FC) {
    if ((hal_millis() - s_obdState.isoTpState.fcWaitStart) >=
    ...
    for (size_t drain = 0; drain < 8; drain++) {
      ...
      if (gotFrame && idMatches) {
        if (s_obdState.dlcValue >= 3 &&
            ((s_obdState.rxBufValue[0] & 0xF0) == 0x30)) {
          ...
          hal_deb_hex("ISO-TP lost frame", s_obdState.rxBufValue,
                      (s_obdState.dlcValue < 8) ? s_obdState.dlcValue : 8, 8);
        }
      }
  ...
  if (s_obdState.isoTpState.lastCfTime != 0 &&
      (hal_millis() - s_obdState.isoTpState.lastCfTime) <
  ...
  s_obdState.isoTpState.index =
      (uint8_t)((s_obdState.isoTpState.index + 1) & 0x0F);
  ...
  hal_can_send(s_obdState.canHandle, s_obdState.isoTpState.responseId, 8,
               tpData);
