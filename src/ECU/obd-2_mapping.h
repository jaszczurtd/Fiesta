...
#define DTC_PCF8574_COMM_FAIL                                                  \
  0xC073 // U0073 Control Module Communication Bus Off
#define DTC_PWM_CHANNEL_NOT_INIT                                               \
  0x0657 // P0657 Actuator Supply Voltage "A" Circuit/Open
#define DTC_DPF_COMM_LOST                                                      \
  0xC100 // U0100 Lost communication with DPF module (project mapping)
#define DTC_EGT_COMM_LOST 0xD902 // U1902 Lost communication with EGT module

// Proposed future DTC set (project catalog).
// Note: A code becomes reportable only after adding it to dtcManager.c state
// table.
...
#define DTC_SD_LOGGER_NOT_READY                                                \
  0xD905                            // U1905 SD logger missing/not initialized
...
#define DTC_ECM_EEPROM_FAULT                                                   \
  0x062F // P062F Internal control module EEPROM error
...
#define DTC_THROTTLE_RANGE_PERF                                                \
  0x0121 // P0121 Throttle/Pedal position range/performance
#define DTC_COOLANT_TEMP_RANGE                                                 \
  0x0116 // P0116 Engine coolant temperature range/performance
#define DTC_INTAKE_TEMP_RANGE                                                  \
  0x0111 // P0111 Intake air temperature range/performance
...
// Adjustometer (VP37 feedback module) DTCs - active in dtcManager.c state
// table.
#define DTC_ADJ_COMM_LOST                                                      \
  0xD908 // U1908 Lost communication with Adjustometer module
#define DTC_ADJ_SIGNAL_LOST 0xD909 // U1909 Adjustometer oscillator signal lost
#define DTC_ADJ_FUEL_TEMP_BROKEN                                               \
  0xD90A // U190A Adjustometer fuel temperature sensor fault
#define DTC_ADJ_VOLTAGE_BAD                                                    \
  0xD90B // U190B Adjustometer supply voltage out of range
#define DTC_RPM_IRQ_INIT_FAIL                                                  \
  0xD90C // U190C RPM interrupt core-affinity/init failure
