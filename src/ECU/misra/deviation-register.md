...
| DR-005 | 8.7 | `src/ECU/engineMaps.c`, `src/ECU/vp37.c`, `src/ECU/vp37_current.c` and the further `vp37_*.c` files of the VP37 module split | accepted-deviation | open | Advisory. The engine maps are data with external linkage consumed by one module each by design, so one table can only be edited in one place. The VP37 module is split into thematic translation units whose step functions have a single caller in another unit; folding them back into the caller's file would undo the split. The screening runs over `src/ECU/*.c` only, so functions consumed by the host tests also read as referenced from one unit; an object a file defines without using it, as the maps are, is not reported by this checker at all, so the maps need no suppression. Decided 2026-09-16. | `misra-c2012-8.7:vp37.c`, `misra-c2012-8.7:vp37_current.c` |
