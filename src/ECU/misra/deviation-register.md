...
| DR-005 | 8.7 | `src/ECU/engineMaps.c` and the VP37 module: `vp37.c`, `vp37_feedback.c`, `vp37_compensation.c`, `vp37_control.c`, `vp37_telemetry.c`, `vp37_current.c` | accepted-deviation | open | Advisory. The engine maps are data with external linkage consumed by one module each by design, so one table can only be edited in one place. The VP37 module is split into thematic translation units whose step functions have a single caller in another unit; folding them back into the caller's file would undo the split. The screening runs over `src/ECU/*.c` only, so functions consumed by the host tests also read as referenced from one unit; a symbol a file defines without using it itself, as the maps and the split's step functions are, is not reported by this checker at all, so only the two units that still call their own external functions carry a suppression. Decided 2026-09-16. | `misra-c2012-8.7:vp37.c`, `misra-c2012-8.7:vp37_current.c` |
