...
| DR-005 | 19.2 | `src/ECU/vp37_current.c` | accepted-deviation | open | The bench current telemetry overlays its 4096-bin ADC histogram and its 1024-entry phase-capture buffer in one union. The two modes are mutually exclusive by construction: an aggregate window reduces samples as they arrive and never keeps them, a phase capture keeps them raw and never bins them. Every entry point resets the storage before switching mode, so no path reads one member after writing the other. Separate buffers would add 8 KiB of static RAM to a bench-only feature. | none |
