...
static float engineOperation_computeAfterstartScale(int32_t coolant) {
  ...
  return (float)(ENGINE_OP_AFTERSTART_TEMP_HIGH - coolant) /
         (float)(ENGINE_OP_AFTERSTART_TEMP_HIGH -
                 ENGINE_OP_AFTERSTART_TEMP_LOW);
}

static void engineOperation_startAfterstart(engineOperation *self,
                                            uint32_t nowMs) {
  int32_t coolant = (int32_t)getGlobalValue(F_COOLANT_TEMP);
  ...
  self->afterStartDurationMs =
      (uint32_t)((float)ENGINE_OP_AFTERSTART_DURATION_WARM_MS +
                 (durationRange * scale));
  self->afterStartStartMs = nowMs;
  self->afterstartActive =
      (self->afterStartDurationMs > 0u) && (self->afterStartBaseAdd > 0.0f);
}
...
static float engineOperation_getAfterstartAdder(engineOperation *self,
                                                uint32_t nowMs) {
...
static float engineOperation_updateStartDemand(engineOperation *self,
                                               uint32_t nowMs) {
...
static float engineOperation_updateIdleDemand(engineOperation *self,
                                              int32_t rpm, int32_t targetRpm,
                                              uint32_t nowMs) {
  ...
  if (elapsed >= ENGINE_OP_IDLE_UPDATE_INTERVAL_MS) {
    ...
    self->idleIntegrator = hal_constrain(
        self->idleIntegrator, -ENGINE_OP_IDLE_I_LIMIT, ENGINE_OP_IDLE_I_LIMIT);
    self->lastIdleUpdateMs = nowMs;
  }

  float error = (float)(targetRpm - rpm);
  float demand = (float)ENGINE_OP_IDLE_BASE_DEMAND_PERCENT +
                 (ENGINE_OP_IDLE_P_GAIN * error) + self->idleIntegrator;

  return hal_constrain(demand, (float)ENGINE_OP_IDLE_DEMAND_MIN,
                       (float)ENGINE_OP_IDLE_DEMAND_MAX);
}

static float engineOperation_applyRampDown(engineOperation *self,
...
void engineOperation_process(engineOperation *self) {
  ...
  bool throttleClosed =
      (driverDemand <= ACCELERATE_MIN_PERCENTAGE_THROTTLE_VALUE);
  ...
  if (!engineRunning && !engineCranking) {
    ...
    self->startDemand = ENGINE_OP_START_DEMAND_MIN;
    requested = engineOperation_updateIdleDemand(
        self, rpm, engineOperation_getIdleTargetRpm(), nowMs);
  } else {
...
void engineOperation_showDebug(const engineOperation *self) {
  ...
  if (now - lastPeriodicLogMs >= ENGINE_OP_DEBUG_UPDATE_MS) {
    ...
    int32_t rpm = RPM_getCurrentRPM(getRPMInstance());
    bool crankingDetected =
        (rpm > ENGINE_OP_CRANKING_RPM_MIN) && (rpm < RPM_MIN);
