...
static const PhaseFraction k_phase_fractions[] = {
    {SC_FLASH_PHASE_FORMAT_CHECK, SC_UI_FLASH_PHASE_FORMAT_CHECK_START,
     SC_UI_FLASH_PHASE_FORMAT_CHECK_EXPECTED_MS},
    {SC_FLASH_PHASE_MANIFEST_VERIFY, SC_UI_FLASH_PHASE_MANIFEST_VERIFY_START,
     SC_UI_FLASH_PHASE_MANIFEST_VERIFY_EXPECTED_MS},
    {SC_FLASH_PHASE_AUTHENTICATE, SC_UI_FLASH_PHASE_AUTHENTICATE_START,
    ...
    {SC_FLASH_PHASE_WAIT_BOOTSEL, SC_UI_FLASH_PHASE_WAIT_BOOTSEL_START,
     SC_UI_FLASH_PHASE_WAIT_BOOTSEL_EXPECTED_MS},
    {SC_FLASH_PHASE_COPY, SC_UI_FLASH_PHASE_COPY_START,
     SC_UI_FLASH_PHASE_COPY_EXPECTED_MS},
    {SC_FLASH_PHASE_WAIT_REENUM, SC_UI_FLASH_PHASE_WAIT_REENUM_START,
     SC_UI_FLASH_PHASE_WAIT_REENUM_EXPECTED_MS},
    {SC_FLASH_PHASE_POST_FLASH_HELLO, SC_UI_FLASH_PHASE_POST_FLASH_HELLO_START,
     SC_UI_FLASH_PHASE_POST_FLASH_HELLO_EXPECTED_MS},
};
static const size_t k_phase_fractions_count = COUNTOF(k_phase_fractions);
...
static void section_show_progress_result(ScFlashSection *s, bool ok) {
  if (s == NULL || s->progress_slot == NULL ||
...
static void start_phase_creep(ScFlashSection *s, ScFlashPhase phase,
  ...
  s->creep_timer_id =
      g_timeout_add(SC_UI_FLASH_CREEP_TICK_MS, on_creep_tick, s);
}
...
static sc_flash_status_t check_uf2_format(const char *path, char *detail,
                                          size_t detail_size) {
...
static void run_manifest_verify(ScFlashSection *s, const char *manifest_path) {
  ...
  const sc_manifest_status_t parse_st =
      sc_manifest_load_file(manifest_path, &m);
  ...
  if (path_st != SC_MANIFEST_OK) {
    ...
    if (path_st == SC_MANIFEST_ERR_UF2_FILE_MISSING) {
      section_set_status(
          s, sc_i18n_string_get(SC_I18N_FLASH_STATUS_MANIFEST_NEEDS_UF2));
    } else {
      char buf[512];
      (void)snprintf(
          buf, sizeof(buf),
  ...
  if (fmt_st != SC_FLASH_OK) {
    ...
    (void)snprintf(buf, sizeof(buf),
                   ...
                   sc_flash_status_str(fmt_st), fmt_detail);
    section_set_status(s, buf);
    return;
  }
...
static void section_recompute_status(ScFlashSection *s) {
  if (s == NULL || s->state == NULL)
    return;
  const char *manifest =
      sc_flash_paths_get_manifest(&s->state->flash_paths, s->module_name);
...
dialog_initial_folder_from_module_build(const ScFlashSection *section) {
  ...
  for (size_t i = 0u; i < root_count; ++i) {
    char *candidate = g_build_filename(k_roots[i], module_dir, ".build", NULL);
...
static GFile *
dialog_initial_folder_for_manifest(const ScFlashSection *section) {
  ...
  const char *secondary = sc_flash_paths_get_uf2(&section->state->flash_paths,
                                                 section->module_name);
...
static void open_manifest_file_dialog(ScFlashSection *section) {
  ...
  gtk_file_dialog_set_title(
      dialog, sc_i18n_string_get(SC_I18N_FLASH_DIALOG_MANIFEST_TITLE));
  ...
  ctx->section = section;
  gtk_file_dialog_open(dialog, parent, NULL, on_manifest_dialog_open_finish,
                       ctx);
...
static void open_manifest_file_dialog(ScFlashSection *section) {
  ...
  section_set_status(
      section, "GtkFileDialog requires GTK 4.10+; rebuild on a newer host.");
}
#endif
...
static gboolean flash_idle_apply(gpointer user_data) {
  ...
  if (ev->kind == FLASH_EV_PROGRESS) {
    ...
    if (ev->phase == SC_FLASH_PHASE_COPY && ev->bytes_total > 0u) {
      ...
      const double phase_start = phase_start_fraction(SC_FLASH_PHASE_COPY);
      const double phase_end = phase_end_fraction(SC_FLASH_PHASE_COPY);
      const double in_phase =
          (double)ev->bytes_written / (double)ev->bytes_total;
      const double target = phase_start + (phase_end - phase_start) * in_phase;
      ...
      creep_from = frac;
      const unsigned pct =
          (unsigned)((ev->bytes_written * 100u) / ev->bytes_total);
      (void)snprintf(buf, sizeof(buf),
    ...
    if (s->module_index < SC_MODULE_COUNT) {
      s_last_flash_result[s->module_index] = (ev->status == SC_FLASH_STATUS_OK)
                                                 ? SC_LAST_FLASH_OK
                                                 : SC_LAST_FLASH_FAIL;
    }
...
static void flash_post_to_ui(FlashEvent *ev) {
  ...
  g_main_context_invoke_full(NULL, G_PRIORITY_DEFAULT_IDLE, flash_idle_apply,
                             ev, NULL);
}
...
static gpointer flash_worker_main(gpointer user_data) {
  ...
  char err[512] = {0};
  const char *manifest =
      (ctx->manifest_path[0] != '\0') ? ctx->manifest_path : NULL;
  const ScFlashStatus rc =
      sc_core_flash(ctx->transport, ctx->module_index, ctx->device_path,
                    ctx->uid_hex, ctx->uf2_path, manifest, NULL,
                    flash_progress_marshal, ctx, err, sizeof(err));
...
static void on_flash_clicked(GtkButton *button, gpointer user_data) {
  ...
  const ScModuleStatus *status =
      sc_core_module_status(&s->state->core, s->module_index);
  if (status == NULL || !status->detected || status->port_path[0] == '\0' ||
      !status->hello_identity.valid || status->hello_identity.uid[0] == '\0') {
  ...
  const char *manifest =
      sc_flash_paths_get_manifest(&s->state->flash_paths, s->module_name);
  if (manifest == NULL || manifest[0] == '\0') {
    section_set_status(s, sc_i18n_string_get(SC_I18N_FLASH_STATUS_NEED_UF2));
    return;
  }
  ...
  run_manifest_verify(s, manifest);
  const char *uf2 =
      sc_flash_paths_get_uf2(&s->state->flash_paths, s->module_name);
  if (uf2 == NULL || uf2[0] == '\0') {
  ...
  (void)snprintf(ctx->manifest_path, sizeof(ctx->manifest_path), "%s",
                 manifest);
  ...
  GThread *worker = g_thread_new("sc_flash_worker", flash_worker_main, ctx);
...
static GtkWidget *build_section(AppState *state, size_t module_index) {
  ...
  GtkWidget *m_lbl =
      gtk_label_new(sc_i18n_string_get(SC_I18N_FLASH_LBL_MANIFEST));
  ...
  s->flash_btn =
      gtk_button_new_with_label(sc_i18n_string_get(SC_I18N_FLASH_BTN_FLASH));
  g_signal_connect(s->flash_btn, "clicked", G_CALLBACK(on_flash_clicked), s);
...
void sc_flash_tab_refresh_sensitivity(AppState *state) {
  ...
  gtk_widget_set_sensitive(state->flash_tab_root,
...
void sc_flash_tab_rebuild_sections(AppState *state) {
  ...
  if (!any_in_scope_module_detected(state)) {
    GtkWidget *placeholder =
        gtk_label_new(sc_i18n_string_get(SC_I18N_FLASH_PLACEHOLDER));
...
void sc_flash_tab_set_lock(AppState *state, bool locked) {
  ...
  for (size_t i = 0u; i < SC_MODULE_COUNT; ++i) {
    ...
    if (s->manifest_pick_btn != NULL)
      gtk_widget_set_sensitive(s->manifest_pick_btn, sensitive);
    if (s->manifest_clear_btn != NULL)
      gtk_widget_set_sensitive(s->manifest_clear_btn, sensitive);
    if (s->flash_btn != NULL)
      gtk_widget_set_sensitive(s->flash_btn, sensitive);
