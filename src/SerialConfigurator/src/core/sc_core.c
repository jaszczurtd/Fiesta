...
static void log_append(char *log_output, size_t log_output_size,
  ...
  (void)vsnprintf(log_output + current_len, log_output_size - current_len,
                  format, args);
  va_end(args);
}
...
static bool decode_build_base64_relaxed(const char *value, char *decoded,
                                        size_t decoded_size) {
  ...
  while (trailing_padding < value_len &&
         normalized[value_len - 1u - trailing_padding] == '=') {
    trailing_padding++;
  }
  if (trailing_padding > 2u) {
    return false;
  ...
  const bool decoded_ok =
      sc_crypto_base64_decode(normalized, normalized_len, (uint8_t *)decoded,
                              decoded_size - 1u, &decoded_len);
  if (!decoded_ok || decoded_len == 0u ||
      !text_is_printable_ascii(decoded, decoded_len)) {
    return false;
  }
...
static void identity_set_field(ScIdentityData *identity, const char *key,
                               const char *value) {
...
static void parse_identity_fields(const char *response,
                                  ScIdentityData *identity) {
  if (identity == 0) {
    return;
  }
  ...
  static const char *k_fields[] = {"module", "proto", "session",
                                   "fw",     "build", "uid"};
  ...
  for (size_t fi = 0u; fi < field_count; ++fi) {
    ...
    for (const char *p = response; p[0] != '\0'; ++p) {
      const bool boundary =
          (p == response) || is_identity_separator_char(p[-1]);
      if (!boundary) {
        continue;
      }
      ...
      if (is_build && p[key_len] != '\0' &&
          !is_identity_separator_char(p[key_len])) {
    ...
    for (const char *scan = value_start + 1u; scan[0] != '\0'; ++scan) {
      ...
      for (size_t next_i = 0u; next_i < field_count; ++next_i) {
        ...
        if (candidate[next_key_len] == '=' ||
            (next_is_build && candidate[next_key_len] != '\0' &&
  ...
  found_value_end:
    while (value_end > value_start &&
           is_identity_separator_char(value_end[-1])) {
      value_end--;
    }
...
static bool parse_hello_identity(const char *response,
                                 ScIdentityData *identity) {
...
static bool parse_meta_identity(const char *response,
                                ScIdentityData *identity) {
...
static bool param_id_char_is_valid(char c) {
  ...
  return (uc >= (unsigned char)'a' && uc <= (unsigned char)'z') ||
         ...
         (uc >= (unsigned char)'0' && uc <= (unsigned char)'9') || c == '_' ||
         c == '-' || c == '.';
}
...
static bool sc_core_send_sc_command_internal(ScCore *core, size_t module_index,
  ...
  if (result == 0) {
    log_append(log_output, log_output_size,
               "[ERROR] Command result buffer is NULL.\n");
    return false;
  }
  ...
  if (module_index >= SC_MODULE_COUNT) {
    log_append(log_output, log_output_size,
               "[ERROR] Module index out of range: %zu\n", module_index);
    return false;
  }
  ...
  if (status->target_ambiguous) {
    log_append(log_output, log_output_size,
               "[ERROR] Refusing command '%s' for '%s': ambiguous target (%zu "
               "instances).\n",
               command, status->display_name, status->detected_instances);
    return false;
  }
  ...
  if (!sc_transport_send_sc_command(&core->transport, status->port_path,
                                    command, response, sizeof(response), error,
                                    sizeof(error))) {
    log_append(log_output, log_output_size, "[ERROR] %s on %s failed: %s\n",
               command, status->port_path, error);
    return false;
  }

  parse_sc_command_result(response, result);
  log_append(log_output, log_output_size, "%s on %s -> %s\n", command,
             status->port_path, result->response);
  return true;
}
...
void sc_core_detect_modules(ScCore *core, char *log_output,
  ...
  sc_core_reset_detection(core);
  log_append(log_output, log_output_size,
             "Detecting Fiesta modules with HELLO...\n");

  ScTransportCandidateList candidates;
  char list_error[256];
  list_error[0] = '\0';

  if (!sc_transport_list_candidates(&core->transport, &candidates, list_error,
                                    sizeof(list_error))) {
    log_append(log_output, log_output_size,
  ...
  for (size_t i = 0u; i < candidates.count; ++i) {
    ...
    if (!sc_transport_resolve_device_path(
            &core->transport, candidate_path, device_path, sizeof(device_path),
            resolve_error, sizeof(resolve_error))) {
      log_append(log_output, log_output_size,
                 "\n[%zu/%zu] %s -> resolve failed: %s\n", i + 1u,
                 candidates.count, candidate_path, resolve_error);
      continue;
    }

    log_append(log_output, log_output_size, "\n[%zu/%zu] %s -> %s\n", i + 1u,
               candidates.count, candidate_path, device_path);
    ...
    if (!sc_transport_send_hello(&core->transport, device_path, response,
    ...
    const int module_index =
        module_index_from_token(parsed_identity.module_name);
    if (module_index < 0) {
      log_append(log_output, log_output_size,
    ...
    if (status->detected) {
      ...
      log_append(log_output, log_output_size,
                 "Module '%s' appears multiple times (%zu instances). Marked "
                 "ambiguous.\n",
                 status->display_name, status->detected_instances);
    }
    ...
    log_append(log_output, log_output_size, "Detected module '%s' on %s\n",
               status->display_name, device_path);
  ...
  for (size_t i = 0u; i < SC_MODULE_COUNT; ++i) {
    ...
    log_append(log_output, log_output_size, "- %-12s : %s (instances=%zu%s)\n",
               status->display_name,
...
bool sc_core_sc_get_meta(ScCore *core, size_t module_index,
  ...
  const bool success = sc_core_send_sc_command_internal(
      core, module_index, SC_CMD_GET_META, result, log_output, log_output_size);
...
bool sc_core_sc_get_param_list(ScCore *core, size_t module_index,
                               ScCommandResult *result, char *log_output,
                               size_t log_output_size) {
  return sc_core_send_sc_command_internal(core, module_index,
                                          SC_CMD_GET_PARAM_LIST, result,
...
bool sc_core_sc_get_values(ScCore *core, size_t module_index,
                           ScCommandResult *result, char *log_output,
                           size_t log_output_size) {
  return sc_core_send_sc_command_internal(core, module_index, SC_CMD_GET_VALUES,
                                          result, log_output, log_output_size);
}

bool sc_core_sc_bye(ScCore *core, size_t module_index, ScCommandResult *result,
                    char *log_output, size_t log_output_size) {
  return sc_core_send_sc_command_internal(core, module_index, SC_CMD_BYE,
                                          result, log_output, log_output_size);
}
...
bool sc_core_sc_get_param(ScCore *core, size_t module_index,
  ...
  const int written =
      snprintf(command, sizeof(command), SC_CMD_GET_PARAM " %s", param_id);
  ...
  return sc_core_send_sc_command_internal(core, module_index, command, result,
                                          log_output, log_output_size);
}
...
static bool extract_kv_value(const char *src, const char *key, char *out,
                             size_t out_size) {
...
const char *sc_auth_status_name(ScAuthStatus status) {
  ...
  case SC_AUTH_OK:
    return "OK";
  case SC_AUTH_ERR_NULL_ARG:
    return "NULL_ARG";
  case SC_AUTH_ERR_HELLO_FAILED:
    return "HELLO_FAILED";
  case SC_AUTH_ERR_HELLO_PARSE:
    return "HELLO_PARSE";
  case SC_AUTH_ERR_BEGIN_FAILED:
    return "BEGIN_FAILED";
  case SC_AUTH_ERR_BAD_CHALLENGE:
    return "BAD_CHALLENGE";
  case SC_AUTH_ERR_RESPONSE_COMPUTE:
    return "RESPONSE_COMPUTE";
  case SC_AUTH_ERR_PROVE_FAILED:
    return "PROVE_FAILED";
  case SC_AUTH_ERR_AUTH_REJECTED:
...
const char *sc_reboot_status_name(ScRebootStatus status) {
  ...
  case SC_REBOOT_OK:
    return "OK";
  case SC_REBOOT_ERR_NULL_ARG:
    return "NULL_ARG";
  case SC_REBOOT_ERR_TRANSPORT:
    return "TRANSPORT";
  case SC_REBOOT_ERR_NOT_AUTHORIZED:
    return "NOT_AUTHORIZED";
  case SC_REBOOT_ERR_UNEXPECTED_REPLY:
...
ScAuthStatus sc_core_authenticate(const ScTransport *transport,
  ...
  if (transport->ops->send_hello == NULL ||
      !transport->ops->send_hello(transport->context, device_path, hello,
                                  sizeof(hello), tx_error, sizeof(tx_error))) {
  ...
  if (strlen(uid_hex) != sizeof(uid_bytes) * 2u ||
    ...
    set_phase5_error(error, error_size, "HELLO parse: bad uid hex '%s'",
                     uid_hex);
    return SC_AUTH_ERR_HELLO_PARSE;
  }
  ...
  if (end == session_str || *end != '\0' || session_ul == 0u ||
    ...
    set_phase5_error(error, error_size, "HELLO parse: bad session '%s'",
                     session_str);
    return SC_AUTH_ERR_HELLO_PARSE;
  }
  ...
  if (transport->ops->send_sc_command == NULL ||
      !transport->ops->send_sc_command(
          transport->context, device_path, SC_CMD_AUTH_BEGIN, begin_reply,
          sizeof(begin_reply), tx_error, sizeof(tx_error))) {
    set_phase5_error(error, error_size, "AUTH_BEGIN transport failed: %s",
                     tx_error);
  ...
  if (strncmp(begin_reply, k_chal_prefix, sizeof(k_chal_prefix) - 1u) != 0) {
    set_phase5_error(error, error_size, "AUTH_BEGIN unexpected reply: %s",
                     begin_reply);
  ...
  if (!transport->ops->send_sc_command(
          transport->context, device_path, prove_cmd, prove_reply,
          sizeof(prove_reply), tx_error, sizeof(tx_error))) {
    set_phase5_error(error, error_size, "AUTH_PROVE transport failed: %s",
                     tx_error);
    return SC_AUTH_ERR_PROVE_FAILED;
  }

  if (strcmp(prove_reply, SC_REPLY_AUTH_OK) != 0) {
    set_phase5_error(error, error_size, "AUTH_PROVE rejected: %s", prove_reply);
...
ScRebootStatus sc_core_reboot_to_bootloader(const ScTransport *transport,
  ...
  if (!transport->ops->send_sc_command(
          transport->context, device_path, SC_CMD_REBOOT_BOOTLOADER, reply,
          sizeof(reply), tx_error, sizeof(tx_error))) {
    set_phase5_error(error, error_size,
  ...
  if (strncmp(reply, SC_STATUS_NOT_AUTHORIZED,
    ...
    set_phase5_error(error, error_size, "firmware refused: %s", reply);
...
const char *sc_set_param_status_name(ScSetParamStatus status) {
  switch (status) {
  case SC_SET_PARAM_OK:
    return "OK";
  case SC_SET_PARAM_ERR_NULL_ARG:
    return "NULL_ARG";
  case SC_SET_PARAM_ERR_TRANSPORT:
    return "TRANSPORT";
  case SC_SET_PARAM_ERR_NOT_AUTHORIZED:
    return "NOT_AUTHORIZED";
  case SC_SET_PARAM_ERR_INVALID_ID:
    return "INVALID_ID";
  case SC_SET_PARAM_ERR_READ_ONLY:
    return "READ_ONLY";
  case SC_SET_PARAM_ERR_OUT_OF_RANGE:
    return "OUT_OF_RANGE";
  case SC_SET_PARAM_ERR_UNEXPECTED_REPLY:
    return "UNEXPECTED_REPLY";
  }
  return "UNKNOWN";
}
...
const char *sc_commit_params_status_name(ScCommitParamsStatus status) {
  switch (status) {
  case SC_COMMIT_PARAMS_OK:
    return "OK";
  case SC_COMMIT_PARAMS_ERR_NULL_ARG:
    return "NULL_ARG";
  case SC_COMMIT_PARAMS_ERR_TRANSPORT:
    return "TRANSPORT";
  case SC_COMMIT_PARAMS_ERR_NOT_AUTHORIZED:
    return "NOT_AUTHORIZED";
  case SC_COMMIT_PARAMS_ERR_COMMIT_FAILED:
    return "COMMIT_FAILED";
  case SC_COMMIT_PARAMS_ERR_UNEXPECTED_REPLY:
    return "UNEXPECTED_REPLY";
  }
  return "UNKNOWN";
}
...
const char *sc_revert_params_status_name(ScRevertParamsStatus status) {
  switch (status) {
  case SC_REVERT_PARAMS_OK:
    return "OK";
  case SC_REVERT_PARAMS_ERR_NULL_ARG:
    return "NULL_ARG";
  case SC_REVERT_PARAMS_ERR_TRANSPORT:
    return "TRANSPORT";
  case SC_REVERT_PARAMS_ERR_NOT_AUTHORIZED:
    return "NOT_AUTHORIZED";
  case SC_REVERT_PARAMS_ERR_UNEXPECTED_REPLY:
    return "UNEXPECTED_REPLY";
  }
  return "UNKNOWN";
}
...
ScSetParamStatus sc_core_set_param(const ScTransport *transport,
  ...
  const int n = snprintf(cmd, sizeof(cmd), SC_CMD_SET_PARAM " %s %d", param_id,
                         (int)value);
  if (n < 0 || (size_t)n >= sizeof(cmd)) {
    set_phase5_error(error, error_size, SC_CMD_SET_PARAM " command too long");
    return SC_SET_PARAM_ERR_NULL_ARG;
  }
  ...
  tx_error[0] = '\0';
  char reply[SC_HELLO_RESPONSE_MAX];
  if (!transport->ops->send_sc_command(transport->context, device_path, cmd,
                                       reply, sizeof(reply), tx_error,
                                       sizeof(tx_error))) {
    set_phase5_error(error, error_size,
  ...
  static const char k_ok_prefix[] = SC_STATUS_OK " " SC_REPLY_TAG_PARAM_SET " ";
...
ScCommitParamsStatus sc_core_commit_params(const ScTransport *transport,
  ...
  char reply[SC_HELLO_RESPONSE_MAX];
  if (!transport->ops->send_sc_command(
          transport->context, device_path, SC_CMD_COMMIT_PARAMS, reply,
          sizeof(reply), tx_error, sizeof(tx_error))) {
    set_phase5_error(error, error_size,
                     SC_CMD_COMMIT_PARAMS " transport failed: %s", tx_error);
...
ScRevertParamsStatus sc_core_revert_params(const ScTransport *transport,
  ...
  char reply[SC_HELLO_RESPONSE_MAX];
  if (!transport->ops->send_sc_command(
          transport->context, device_path, SC_CMD_REVERT_PARAMS, reply,
          sizeof(reply), tx_error, sizeof(tx_error))) {
    set_phase5_error(error, error_size,
                     SC_CMD_REVERT_PARAMS " transport failed: %s", tx_error);
...
const char *sc_flash_status_name(ScFlashStatus status) {
  switch (status) {
  case SC_FLASH_STATUS_OK:
    return "OK";
  case SC_FLASH_STATUS_NULL_ARG:
    return "NULL_ARG";
  case SC_FLASH_STATUS_FORMAT_REJECTED:
    return "FORMAT_REJECTED";
  case SC_FLASH_STATUS_MANIFEST_PARSE_FAILED:
    return "MANIFEST_PARSE_FAILED";
  case SC_FLASH_STATUS_MANIFEST_MODULE_MISMATCH:
    return "MANIFEST_MODULE_MISMATCH";
  case SC_FLASH_STATUS_MANIFEST_ARTIFACT_MISMATCH:
    return "MANIFEST_ARTIFACT_MISMATCH";
  case SC_FLASH_STATUS_AUTH_FAILED:
    return "AUTH_FAILED";
  case SC_FLASH_STATUS_REBOOT_FAILED:
    return "REBOOT_FAILED";
  case SC_FLASH_STATUS_BOOTSEL_TIMEOUT:
    return "BOOTSEL_TIMEOUT";
  case SC_FLASH_STATUS_COPY_FAILED:
    return "COPY_FAILED";
  case SC_FLASH_STATUS_REENUM_TIMEOUT:
    return "REENUM_TIMEOUT";
  case SC_FLASH_STATUS_POST_FLASH_HELLO_FAILED:
    return "POST_FLASH_HELLO_FAILED";
  case SC_FLASH_STATUS_POST_FLASH_FW_MISMATCH:
    return "POST_FLASH_FW_MISMATCH";
  }
  return "UNKNOWN";
}
...
const char *sc_flash_phase_name(ScFlashPhase phase) {
  switch (phase) {
  case SC_FLASH_PHASE_FORMAT_CHECK:
    return "FORMAT_CHECK";
  case SC_FLASH_PHASE_MANIFEST_VERIFY:
    return "MANIFEST_VERIFY";
  case SC_FLASH_PHASE_AUTHENTICATE:
    return "AUTHENTICATE";
  case SC_FLASH_PHASE_REBOOT_TO_BOOTLOADER:
    return "REBOOT_TO_BOOTLOADER";
  case SC_FLASH_PHASE_WAIT_BOOTSEL:
    return "WAIT_BOOTSEL";
  case SC_FLASH_PHASE_COPY:
    return "COPY";
  case SC_FLASH_PHASE_WAIT_REENUM:
    return "WAIT_REENUM";
  case SC_FLASH_PHASE_POST_FLASH_HELLO:
    return "POST_FLASH_HELLO";
  }
  return "UNKNOWN";
}
...
ScFlashStatus sc_core_flash(const ScTransport *transport, size_t module_index,
  ...
  const sc_flash_status_t fmt_st =
      sc_flash_uf2_format_check(uf2_path, helper_err, sizeof(helper_err));
  if (fmt_st != SC_FLASH_OK) {
    flash_set_error(error_buf, error_size, "UF2 format rejected: %s - %s",
  ...
  if (manifest_path_or_null != NULL && manifest_path_or_null[0] != '\0') {
    ...
    const sc_manifest_status_t parse_st =
        sc_manifest_load_file(manifest_path_or_null, &manifest);
    if (parse_st != SC_MANIFEST_OK) {
      flash_set_error(error_buf, error_size, "manifest parse failed: %s",
    ...
    const sc_manifest_status_t mod_st =
        sc_manifest_check_module_match(&manifest, expected);
    ...
    const sc_manifest_status_t art_st =
        sc_manifest_verify_artifact(&manifest, uf2_path);
    if (art_st != SC_MANIFEST_OK) {
      flash_set_error(error_buf, error_size, "manifest artifact mismatch: %s",
  ...
  if (auth_st != SC_AUTH_OK) {
    flash_set_error(error_buf, error_size, "auth failed: %s - %s",
  ...
  if (reboot_st != SC_REBOOT_OK) {
    flash_set_error(error_buf, error_size, "reboot failed: %s - %s",
  ...
  const ScFlashOptions *opts =
      (options_or_null != NULL) ? options_or_null : &defaults;
  ...
  if (opts->bootsel_parents[0] != NULL || opts->bootsel_parents[1] != NULL) {
    const char *parents[2] = {opts->bootsel_parents[0],
                              opts->bootsel_parents[1]};
    const size_t parent_count = (parents[1] != NULL)   ? 2u
                                : (parents[0] != NULL) ? 1u
                                                       : 0u;
    ...
    bs_st = sc_flash__watch_for_bootsel_in(p, parent_count, bootsel_timeout,
                                           bootsel_path, sizeof(bootsel_path),
                                           helper_err, sizeof(helper_err));
  } else {
    bs_st = sc_flash_watch_for_bootsel(bootsel_timeout, bootsel_path,
                                       sizeof(bootsel_path), helper_err,
                                       sizeof(helper_err));
  }
  if (bs_st != SC_FLASH_OK) {
    flash_set_error(error_buf, error_size, "BOOTSEL watcher: %s - %s",
                    sc_flash_status_str(bs_st), helper_err);
    return SC_FLASH_STATUS_BOOTSEL_TIMEOUT;
  }

  /* 6. Copy UF2. */
  flash_copy_progress_state_t copy_state = {.cb = progress_cb,
                                            .user = progress_user};
  helper_err[0] = '\0';
  const sc_flash_status_t cp_st =
      sc_flash_copy_uf2(uf2_path, bootsel_path, flash_copy_progress_adapter,
                        &copy_state, helper_err, sizeof(helper_err));
  if (cp_st != SC_FLASH_OK) {
    flash_set_error(error_buf, error_size, "copy failed: %s - %s",
  ...
  helper_err[0] = '\0';
  const sc_flash_status_t re_st =
      (opts->by_id_parent != NULL)
          ? sc_flash__wait_reenumeration_in(
                opts->by_id_parent, uid_hex, reenum_timeout, new_device_path,
                sizeof(new_device_path), helper_err, sizeof(helper_err))
          : sc_flash_wait_reenumeration(
                uid_hex, reenum_timeout, new_device_path,
                sizeof(new_device_path), helper_err, sizeof(helper_err));
  if (re_st != SC_FLASH_OK) {
    flash_set_error(error_buf, error_size, "re-enumeration: %s - %s",
  ...
  char hello_err[256];
  if (!sc_transport_send_hello(transport, new_device_path, hello_response,
                               sizeof(hello_response), hello_err,
                               sizeof(hello_err))) {
    flash_set_error(error_buf, error_size, "post-flash HELLO transport: %s",
                    hello_err);
  ...
  if (!parse_hello_identity(hello_response, &parsed)) {
    flash_set_error(error_buf, error_size,
                    "post-flash HELLO unparseable: %.180s", hello_response);
    return SC_FLASH_STATUS_POST_FLASH_HELLO_FAILED;
  }
  ...
  flash_set_error(error_buf, error_size, "%s flashed, fw=%s, build=%s",
