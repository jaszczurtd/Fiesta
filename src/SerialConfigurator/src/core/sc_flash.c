...
const char *sc_flash_status_str(sc_flash_status_t st) {
  ...
  case SC_FLASH_OK:
    return "OK";
  case SC_FLASH_ERR_NULL_ARG:
    return "NULL_ARG";
  case SC_FLASH_ERR_FILE_OPEN:
    return "FILE_OPEN";
  case SC_FLASH_ERR_FILE_READ:
    return "FILE_READ";
  case SC_FLASH_ERR_EMPTY:
    return "EMPTY";
  case SC_FLASH_ERR_TOO_LARGE:
    return "TOO_LARGE";
  case SC_FLASH_ERR_NOT_BLOCK_ALIGNED:
    return "NOT_BLOCK_ALIGNED";
  case SC_FLASH_ERR_BAD_FIRST_MAGIC:
    return "BAD_FIRST_MAGIC";
  case SC_FLASH_ERR_BAD_SECOND_MAGIC:
    return "BAD_SECOND_MAGIC";
  case SC_FLASH_ERR_BAD_END_MAGIC:
    return "BAD_END_MAGIC";
  case SC_FLASH_ERR_WRONG_FAMILY:
    return "WRONG_FAMILY";
  case SC_FLASH_ERR_BLOCK_INDEX_OUT_OF_RANGE:
    return "BLOCK_INDEX_OUT_OF_RANGE";
  case SC_FLASH_ERR_BOOTSEL_TIMEOUT:
    return "BOOTSEL_TIMEOUT";
  case SC_FLASH_ERR_FILE_WRITE:
    return "FILE_WRITE";
  case SC_FLASH_ERR_REENUM_TIMEOUT:
    return "REENUM_TIMEOUT";
  case SC_FLASH_ERR_NOT_IMPLEMENTED:
...
sc_flash_status_t sc_flash_uf2_format_check(const char *path, char *error_buf,
                                            size_t error_size) {
  if (path == NULL) {
  ...
  if (len > SC_FLASH_UF2_MAX_BYTES) {
    (void)fclose(f);
    write_error(error_buf, error_size, "file is %zu bytes, cap is %u", len,
                SC_FLASH_UF2_MAX_BYTES);
    return SC_FLASH_ERR_TOO_LARGE;
  }
  if ((len % SC_FLASH_UF2_BLOCK_SIZE) != 0u) {
    (void)fclose(f);
    write_error(error_buf, error_size, "file size %zu is not a multiple of %u",
  ...
  for (size_t b = 0u; b < total_blocks_seen; ++b) {
    ...
    if (magic1 != SC_FLASH_UF2_FIRST_MAGIC) {
      ...
      write_error(error_buf, error_size,
                  "block %zu: first magic 0x%08X != 0x%08X", b, magic1,
                  SC_FLASH_UF2_FIRST_MAGIC);
    ...
    if (magic2 != SC_FLASH_UF2_SECOND_MAGIC) {
      free(buf);
      write_error(error_buf, error_size,
                  "block %zu: second magic 0x%08X != 0x%08X", b, magic2,
                  SC_FLASH_UF2_SECOND_MAGIC);
    ...
    if (end_magic != SC_FLASH_UF2_END_MAGIC) {
      free(buf);
      write_error(error_buf, error_size,
                  "block %zu: end magic 0x%08X != 0x%08X", b, end_magic,
                  SC_FLASH_UF2_END_MAGIC);
    ...
    if (family != SC_FLASH_UF2_FAMILY_RP2040) {
      free(buf);
      write_error(error_buf, error_size,
                  "block %zu: family id 0x%08X != RP2040 (0x%08X)", b, family,
                  SC_FLASH_UF2_FAMILY_RP2040);
    ...
    if (block_index >= block_total) {
      free(buf);
      write_error(error_buf, error_size, "block %zu: index %u >= total %u", b,
  ...
  free(buf);
  write_error(error_buf, error_size, "%zu blocks, family RP2040",
              total_blocks_seen);
  return SC_FLASH_OK;
}
...
static void parent_obs_reset(parent_obs_t *o) { memset(o, 0, sizeof(*o)); }
...
static void parent_obs_record(parent_obs_t *o, const char *name) {
  ...
  if (o->captured < SC_FLASH_OBS_CAPTURED_MAX) {
    ...
    (void)snprintf(o->first_entries[o->captured], sizeof(o->first_entries[0]),
                   "%.*s", (int)(sizeof(o->first_entries[0]) - 1u), name);
    o->captured++;
  }
}
...
static size_t parent_obs_append(char *dest, size_t dest_size, size_t off,
  ...
  if (!o->exists) {
    n = snprintf(dest + off, dest_size - off,
                 "; parent='%s' opendir_errno=%d (%s)", parent, o->open_errno,
                 strerror(o->open_errno));
  } else if (o->entry_count == 0u) {
    n = snprintf(dest + off, dest_size - off, "; parent='%s' exists empty",
                 parent);
  } else {
    n = snprintf(dest + off, dest_size - off,
                 "; parent='%s' exists entries=%zu [", parent, o->entry_count);
    if (n > 0 && (size_t)n < dest_size - off) {
      off += (size_t)n;
      for (size_t i = 0u; i < o->captured; ++i) {
        int m = snprintf(dest + off, dest_size - off, "%s%s",
                         (i > 0u) ? "," : "", o->first_entries[i]);
      ...
      if (o->reject_reason[0] != '\0') {
        int r = snprintf(dest + off, dest_size - off, " reject=%s",
                         o->reject_reason);
  ...
  if (o->reject_reason[0] != '\0' && o->exists && o->entry_count == 0u) {
    int r =
        snprintf(dest + off, dest_size - off, " reject=%s", o->reject_reason);
    if (r > 0 && (size_t)r < dest_size - off) {
      off += (size_t)r;
    }
  }
 ...
 * it on timeout. */
static bool bootsel_scan_dir_once(const char *parent, char *out_path,
                                  size_t out_path_size, parent_obs_t *obs) {
  parent_obs_reset(obs);
  DIR *d = opendir(parent);
  if (d == NULL) {
    obs->open_errno = errno;
    flash_log_v("opendir('%s') failed: errno=%d (%s)", parent, obs->open_errno,
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    char candidate[1024];
    const int written =
        snprintf(candidate, sizeof(candidate), "%s/%s", parent, name);
    if (written < 0 || (size_t)written >= sizeof(candidate)) {
      (void)snprintf(obs->reject_reason, sizeof(obs->reject_reason),
                     "'%.40s' path too long", name);
      flash_log("'%s/%s' rejected: composed path exceeds buffer", parent, name);
      continue;
    }
    struct stat st;
    if (stat(candidate, &st) != 0) {
      ...
      flash_log("'%s' rejected: stat failed errno=%d (%s)", candidate, e,
                strerror(e));
      continue;
    }
    if (!S_ISDIR(st.st_mode)) {
      (void)snprintf(obs->reject_reason, sizeof(obs->reject_reason),
                     "'%.40s' not_a_dir mode=0%o", name,
                     (unsigned)(st.st_mode & 0xFFFFu));
      flash_log("'%s' rejected: not a directory (mode=0%o)", candidate,
                (unsigned)(st.st_mode & 0xFFFFu));
      continue;
    }
...
static bool bootsel_label_scan_once(char *out_label, size_t out_label_size,
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    char link_path[1024];
    const int n =
        snprintf(link_path, sizeof(link_path), "%s/%s", k_label_dir, name);
    if (n < 0 || (size_t)n >= sizeof(link_path)) {
      continue;
    }
    struct stat st;
    if (stat(link_path, &st) != 0) {
      flash_log_v("stat('%s') failed: errno=%d (%s)", link_path, errno,
                  strerror(errno));
      continue;
    }
    if (!S_ISBLK(st.st_mode)) {
      flash_log_v("'%s' is not a block device (mode=0%o), ignoring", link_path,
                  (unsigned)(st.st_mode & 0xFFFFu));
      continue;
    }
    if (out_label != NULL && out_label_size > 0u) {
      (void)snprintf(out_label, out_label_size, "%.*s",
                     (int)(out_label_size - 1u), name);
    }
    if (out_dev != NULL && out_dev_size > 0u) {
      (void)snprintf(out_dev, out_dev_size, "%.*s", (int)(out_dev_size - 1u),
                     link_path);
    }
    found = true;
    break;
...
static size_t mountinfo_decode_path(char *p) {
  ...
  while (*r != '\0') {
    if (r[0] == '\\' && r[1] >= '0' && r[1] <= '7' && r[2] >= '0' &&
        r[2] <= '7' && r[3] >= '0' && r[3] <= '7') {
      const unsigned v =
          (unsigned)((r[1] - '0') * 64 + (r[2] - '0') * 8 + (r[3] - '0'));
      *w++ = (char)v;
      r += 4;
    } else {
      *w++ = *r++;
...
static bool bootsel_find_mountpoint_in_mountinfo(char *out_path,
  ...
  if (f == NULL) {
    flash_log_v("fopen('/proc/self/mountinfo') failed: errno=%d (%s)", errno,
                strerror(errno));
    return false;
  }
  ...
  while (fgets(line, sizeof(line), f) != NULL) {
    ...
    if (out_path != NULL && out_path_size > 0u) {
      (void)snprintf(out_path, out_path_size, "%.*s", (int)(out_path_size - 1u),
                     mountpoint);
    }
    if (out_writable != NULL) {
...
sc_flash_status_t sc_flash__watch_for_bootsel_in(
    const char *const *parent_dirs, size_t parent_count, uint32_t timeout_ms,
    char *out_path, size_t out_path_size, char *error_buf, size_t error_size) {
  ...
  for (;;) {
    ...
    for (size_t i = 0u; i < scan_count; ++i) {
      ...
      const bool matched = bootsel_scan_dir_once(parent_dirs[i], out_path,
                                                 out_path_size, &obs[i]);
      ...
      if (!obs[i].exists && !logged_open_errno[i]) {
        flash_log("parent[%zu]='%s' opendir errno=%d (%s)", i, parent_dirs[i],
                  obs[i].open_errno, strerror(obs[i].open_errno));
      ...
      if (obs[i].exists != prev_existed[i]) {
        flash_log("parent[%zu]='%s' state %s->%s", i, parent_dirs[i],
      ...
      if (obs[i].exists && obs[i].entry_count != prev_entries[i]) {
        ...
        snap[0] = '\0';
        off =
            parent_obs_append(snap, sizeof(snap), off, parent_dirs[i], &obs[i]);
        (void)off;
        flash_log("parent[%zu] entry-count %zu->%zu%s", i, prev_entries[i],
                  obs[i].entry_count, snap);
        prev_entries[i] = obs[i].entry_count;
      }

      if (matched) {
        write_error(error_buf, error_size, "found at %s",
    ...
    if (now_ms >= deadline_ms) {
      ...
      size_t off = (size_t)snprintf(
          summary, sizeof(summary), "no BOOTSEL drive after %u ms (iters=%llu)",
          (unsigned)timeout_ms, (unsigned long long)iters);
      for (size_t i = 0u; i < scan_count; ++i) {
        if (parent_dirs[i] == NULL) {
          continue;
        }
        off = parent_obs_append(summary, sizeof(summary), off, parent_dirs[i],
                                &obs[i]);
      }
      write_error(error_buf, error_size, "%s", summary);
      flash_log("BOOTSEL_TIMEOUT - %s", summary);
      flash_log("hint: confirm the BOOTSEL drive auto-mounted under one of the "
                "parents above; "
                "if the drive shows up elsewhere (e.g. /mnt/...) the watcher "
                "will not see it");
...
sc_flash_status_t sc_flash_watch_for_bootsel(uint32_t timeout_ms,
  ...
  for (;;) {
    ...
    for (size_t i = 0u; i < scan_count; ++i) {
      const bool matched =
          bootsel_scan_dir_once(parents[i], out_path, out_path_size, &obs[i]);

      if (!obs[i].exists && !logged_open_errno[i]) {
        flash_log("parent[%zu]='%s' opendir errno=%d (%s)", i, parents[i],
                  obs[i].open_errno, strerror(obs[i].open_errno));
        logged_open_errno[i] = true;
      }
      if (obs[i].exists != prev_existed[i]) {
        flash_log("parent[%zu]='%s' state %s->%s", i, parents[i],
                  prev_existed[i] ? "exists" : "missing",
                  obs[i].exists ? "exists" : "missing");
        prev_existed[i] = obs[i].exists;
      }
      if (obs[i].exists && obs[i].entry_count != prev_entries[i]) {
        ...
        size_t off = 0u;
        snap[0] = '\0';
        off = parent_obs_append(snap, sizeof(snap), off, parents[i], &obs[i]);
        (void)off;
        flash_log("parent[%zu] entry-count %zu->%zu%s", i, prev_entries[i],
                  obs[i].entry_count, snap);
        prev_entries[i] = obs[i].entry_count;
    ...
    if (bootsel_find_mountpoint_in_mountinfo(mi_path, sizeof(mi_path),
      ...
      if (mi_writable) {
        ...
        flash_log("BOOTSEL match (mountinfo): '%s'%s", mi_path,
                  under_user_root ? ""
      ...
      int n2 =
          snprintf(summary + s_off, sizeof(summary) - s_off,
    ...
    char tmp_label[64];
    char tmp_dev[256];
    if (bootsel_label_scan_once(tmp_label, sizeof(tmp_label), tmp_dev,
                                sizeof(tmp_dev))) {
      if (!block_dev_seen) {
        block_dev_seen = true;
        block_dev_first_seen_ms = monotonic_ms();
        ...
        (void)snprintf(block_dev_path, sizeof(block_dev_path), "%.*s",
                       (int)(sizeof(block_dev_path) - 1u), tmp_dev);
      }
      if (!block_dev_logged) {
      ...
      if (automount_enabled && !automount_attempted) {
        ...
        if (elapsed_since_seen >= SC_FLASH_AUTOMOUNT_GRACE_MS) {
          ...
          automount_succeeded = bootsel_attempt_udisks_mount(
              block_dev_path, automount_err, sizeof(automount_err));
    ...
    if (now_ms >= deadline_ms) {
      ...
      size_t off = (size_t)snprintf(
          ...
          (unsigned)timeout_ms, (unsigned long long)iters);
      for (size_t i = 0u; i < scan_count; ++i) {
        off = parent_obs_append(summary, sizeof(summary), off, parents[i],
                                &obs[i]);
      }
      if (block_dev_seen) {
        ...
        if (automount_attempted) {
          n = snprintf(summary + off, sizeof(summary) - off,
                       "; automount %s (%s)",
                       automount_succeeded ? "ok-but-no-mountpoint" : "failed",
                       automount_err);
          (void)n;
        } else {
          n = snprintf(summary + off, sizeof(summary) - off,
...
sc_flash_status_t sc_flash_copy_uf2(const char *src_uf2_path,
  ...
  if (src == NULL) {
    write_error(error_buf, error_size, "could not open source '%s'",
                src_uf2_path);
    return SC_FLASH_ERR_FILE_OPEN;
  }
  ...
  if (total > SC_FLASH_UF2_MAX_BYTES) {
    (void)fclose(src);
    write_error(error_buf, error_size, "source is %zu bytes, cap is %u", total,
                SC_FLASH_UF2_MAX_BYTES);
    return SC_FLASH_ERR_TOO_LARGE;
  }

  char dest_path[1024];
  const int dest_len = snprintf(dest_path, sizeof(dest_path), "%s/%s",
                                drive_path, SC_FLASH_COPY_DEST_FILENAME);
  if (dest_len < 0 || (size_t)dest_len >= sizeof(dest_path)) {
    (void)fclose(src);
    write_error(error_buf, error_size, "drive_path too long for buffer");
    return SC_FLASH_ERR_NULL_ARG;
  }
  ...
  if (dst == NULL) {
    (void)fclose(src);
    write_error(error_buf, error_size, "could not open destination '%s'",
                dest_path);
  ...
  while (copied < total) {
    ...
    if (got != want) {
      (void)fclose(src);
      (void)fclose(dst);
      write_error(error_buf, error_size, "short read at %llu / %zu",
    ...
    if (put != got) {
      (void)fclose(src);
      (void)fclose(dst);
      write_error(error_buf, error_size, "short write at %llu / %zu",
                  (unsigned long long)copied, total);
      return SC_FLASH_ERR_FILE_WRITE;
    }
    copied += (uint64_t)put;
  ...
  write_error(error_buf, error_size, "OK: copied %zu bytes to %s", total,
              dest_path);
  return SC_FLASH_OK;
}

sc_flash_status_t sc_flash__wait_reenumeration_in(
    const char *parent_dir, const char *uid_hex, uint32_t timeout_ms,
    char *out_path, size_t out_path_size, char *error_buf, size_t error_size) {
  ...
  for (;;) {
    ...
    if (d != NULL) {
      ...
      while ((ent = readdir(d)) != NULL) {
        ...
        if (strstr(name, uid_hex) == NULL) {
          flash_log_v("'%s/%s' ignored: does not contain uid='%s'", parent_dir,
                      name, uid_hex);
          continue;
        }
        if (!matched) {
      ...
      if (matched) {
        if (out_path != NULL && out_path_size > 0u) {
          (void)snprintf(out_path, out_path_size, "%s/%s", parent_dir,
                         matched_name);
        }
      ...
      obs.open_errno = errno;
      if (!logged_open_err) {
        flash_log("opendir('%s') failed: errno=%d (%s)", parent_dir,
                  obs.open_errno, strerror(obs.open_errno));
        logged_open_err = true;
      }
    }

    if (obs.exists != prev_existed) {
      flash_log("parent='%s' state %s->%s", parent_dir,
    ...
    if (obs.exists && obs.entry_count != prev_entries) {
      ...
      size_t off = 0u;
      snap[0] = '\0';
      off = parent_obs_append(snap, sizeof(snap), off, parent_dir, &obs);
      (void)off;
      flash_log("parent entry-count %zu->%zu%s", prev_entries, obs.entry_count,
                snap);
    ...
    if (now_ms >= deadline_ms) {
      ...
      size_t off = (size_t)snprintf(
          summary, sizeof(summary),
          "no by-id entry matching uid=%s after %u ms (iters=%llu)", uid_hex,
          (unsigned)timeout_ms, (unsigned long long)iters);
      (void)parent_obs_append(summary, sizeof(summary), off, parent_dir, &obs);
      write_error(error_buf, error_size, "%s", summary);
...
sc_flash_status_t
sc_flash_wait_reenumeration(const char *uid_hex, uint32_t timeout_ms,
                            char *out_path, size_t out_path_size,
                            char *error_buf, size_t error_size) {
#if SC_FLASH_HAVE_POSIX_WATCHER
  return sc_flash__wait_reenumeration_in(SC_FLASH_REENUM_PARENT_DIR, uid_hex,
                                         timeout_ms, out_path, out_path_size,
