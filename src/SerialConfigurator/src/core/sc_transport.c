 ...
 */
static bool read_framed_response_with_deadline(int fd, uint16_t expected_seq,
  ...
  if (clock_gettime(CLOCK_MONOTONIC, &t_start) != 0) {
    (void)snprintf(error, error_size, "clock_gettime failed: %s",
                   strerror(errno));
    return false;
  }
  ...
  while (1) {
    ...
    const long elapsed_ms = (t_now.tv_sec - t_start.tv_sec) * 1000L +
    ...
    if (fail_fast_deadline_ms >= 0) {
      ...
      if (left_ff < left_ms_long)
        left_ms_long = left_ff;
    }
        ...
        if (line_overflow) {
          ...
          if (strncmp(trimmed, SC_FRAME_PREFIX, SC_FRAME_PREFIX_LEN) == 0) {
            ...
            if (tl + 2u <= sizeof(repaired_line)) {
              repaired_line[0] = '$';
              (void)snprintf(repaired_line + 1u, sizeof(repaired_line) - 1u,
                             "%s", trimmed);
          ...
          if (framed != NULL) {
            ...
            if (sc_frame_decode(framed, &got_seq, inner, sizeof(inner))) {
              ...
              if (got_seq == expected_seq) {
                (void)snprintf(payload_out, payload_out_size, "%s", inner);
                return true;
              }
              ...
              if (first_bad_sc_line[0] == '\0') {
                (void)snprintf(first_bad_sc_line, sizeof(first_bad_sc_line),
                               "%.80s", framed);
              }
              transport_log_v("drop frame: decode failed line='%.80s'", framed);
              ...
              if (fail_fast_deadline_ms < 0) {
                ...
                if (clock_gettime(CLOCK_MONOTONIC, &t_ff) == 0) {
                  ...
                  fail_fast_deadline_ms = ff_elapsed + FAIL_FAST_GRACE_MS;
                }
              }
            }
          } else {
            ...
            if (first_non_sc_line[0] == '\0') {
              (void)snprintf(first_non_sc_line, sizeof(first_non_sc_line),
                             "%.80s", trimmed);
            }
  ...
  (void)snprintf(error, error_size,
                 "timeout waiting for framed response (seq=%u, bytes=%zu, "
                 "lines=%zu, non_sc=%zu, bad_sc=%zu, wrong_seq=%zu, "
                 "repaired=%zu, overflow=%zu, first_wrong_seq=%u)",
                 (unsigned)expected_seq, bytes_seen, lines_seen, non_sc_lines,
                 bad_sc_lines, wrong_seq_frames, repaired_missing_dollar,
                 overflow_lines, (unsigned)first_wrong_seq);
  return false;
}
...
static bool open_and_configure_port(const char *device_path, int *fd,
  ...
  *fd = open(device_path, O_RDWR | O_NOCTTY | O_NONBLOCK);
  if (*fd < 0) {
    const int e = errno;
    (void)snprintf(error, error_size, "open(%.120s) failed: %.96s", device_path,
                   strerror(e));
    transport_log("open('%s') failed: errno=%d (%s)", device_path, e,
                  strerror(e));
    return false;
  }
  ...
  if (!configure_serial_port(*fd, error, error_size)) {
    transport_log("configure_serial_port(fd=%d) failed: %s", *fd,
                  error ? error : "(no buf)");
    (void)close(*fd);
    *fd = -1;
    return false;
...
static void close_cached_port_slot(size_t slot) {
  ...
  if (s_cached_ports[slot].in_use && s_cached_ports[slot].fd >= 0) {
    transport_log("cache slot=%zu close fd=%d path='%s'", slot,
                  s_cached_ports[slot].fd, s_cached_ports[slot].device_path);
 ...
 */
static void
reconcile_cached_ports_with_list(const ScTransportCandidateList *list) {
  if (list == NULL) {
    return;
  }
  for (size_t i = 0u; i < SC_TRANSPORT_MAX_CACHED_PORTS; ++i) {
    ...
    if (!still_present) {
      transport_log("reconcile: dropping cached slot=%zu fd=%d "
                    ...
                    i, s_cached_ports[i].fd, s_cached_ports[i].device_path);
...
static bool acquire_cached_port(const char *device_path, int *fd,
  ...
  if (slot >= 0) {
    if (s_cached_ports[slot].fd >= 0) {
      transport_log_v("cache hit slot=%d fd=%d path='%s'", slot,
                      s_cached_ports[slot].fd, device_path);
    ...
    transport_log("cache slot=%d stale (fd<0) for path='%s' - reopening", slot,
                  device_path);
    close_cached_port_slot((size_t)slot);
  }
...
static uint16_t allocate_next_seq(size_t slot) {
  ...
  s_cached_ports[slot].next_seq =
      (uint16_t)((seq == 0xFFFFu) ? 1u : (seq + 1u));
  return seq;
}
 ...
 */
static bool framed_exchange_on_fd(int fd, uint16_t seq, const char *inner,
  ...
  return read_framed_response_with_deadline(fd, seq, timeout_ms, response,
                                            response_size, error, error_size);
}

static bool send_hello_bootstrap_on_fd(int fd, size_t cache_slot,
  ...
  if (!framed_exchange_on_fd(fd, seq, "HELLO", timeout_ms, hello_response,
                             sizeof(hello_response), error, error_size)) {
    return false;
  }

  if (!response_has_prefix(hello_response, SC_REPLY_HELLO_HEAD)) {
    (void)snprintf(error, error_size, "HELLO bootstrap rejected: %.180s",
                   hello_response);
    return false;
  }
...
static bool send_sc_command_on_fd(int fd, size_t cache_slot,
                                  const char *command_inner, int timeout_ms,
                                  char *response, size_t response_size,
                                  char *error, size_t error_size) {
  const uint16_t seq = allocate_next_seq(cache_slot);
  return framed_exchange_on_fd(fd, seq, command_inner, timeout_ms, response,
                               response_size, error, error_size);
}

static bool default_list_candidates(void *context,
  ...
  if (glob_rc != 0) {
    (void)snprintf(error, error_size, "device scan failed (glob rc=%d)",
                   glob_rc);
    globfree(&devices);
    return false;
  }
...
static bool default_resolve_device_path(void *context,
                                        const char *candidate_path,
                                        char *device_path,
                                        size_t device_path_size, char *error,
                                        size_t error_size) {
  (void)context;
  (void)error;
  (void)error_size;
...
static bool default_send_hello(void *context, const char *device_path,
                               char *response, size_t response_size,
                               char *error, size_t error_size) {
  (void)context;
  ...
  for (int attempt = 0; attempt < SC_TRANSPORT_HELLO_ATTEMPTS; ++attempt) {
    ...
    do {
      ...
      if (!framed_exchange_on_fd(fd, seq, "HELLO", timeout_ms, response,
                                 response_size, last_error,
                                 sizeof(last_error))) {
        break;
      }
    ...
    if (success) {
      transport_log("HELLO ok path='%s' attempt=%d reply='%.80s'", device_path,
                    attempt + 1, response);
      return true;
    }

    transport_log("HELLO attempt %d/%d failed path='%s': %s", attempt + 1,
                  SC_TRANSPORT_HELLO_ATTEMPTS, device_path, last_error);
    invalidate_cached_port(device_path);

    if (attempt + 1 < SC_TRANSPORT_HELLO_ATTEMPTS) {
      struct timeval retry_pause = {.tv_sec = 0,
                                    .tv_usec = SC_TRANSPORT_RETRY_PAUSE_USEC};
...
static bool default_send_sc_command(void *context, const char *device_path,
  ...
  for (int attempt = 0; attempt < SC_TRANSPORT_SC_ATTEMPTS; ++attempt) {
    ...
    do {
      ...
      if (!send_sc_command_on_fd(fd, slot, inner, timeout_ms, response,
                                 response_size, last_error,
                                 sizeof(last_error))) {
        break;
      }
    ...
    if (success) {
      if (attempt > 0) {
        ...
        transport_log_v("SC '%s' ok path='%s' attempt=%d reply='%.80s'", inner,
                        device_path, attempt + 1, response);
      }
      return true;
    }

    transport_log("SC '%s' attempt %d/%d failed path='%s': %s", inner,
                  attempt + 1, SC_TRANSPORT_SC_ATTEMPTS, device_path,
                  last_error);
    invalidate_cached_port(device_path);
...
void sc_transport_init_custom(ScTransport *transport, const ScTransportOps *ops,
                              void *context) {
  if (transport == 0) {
    return;
  }
...
bool sc_transport_list_candidates(const ScTransport *transport,
                                  ScTransportCandidateList *list, char *error,
                                  size_t error_size) {
  ...
  return transport->ops->list_candidates(transport->context, list, error,
                                         error_size);
}

bool sc_transport_resolve_device_path(const ScTransport *transport,
                                      const char *candidate_path,
                                      char *device_path,
                                      size_t device_path_size, char *error,
                                      size_t error_size) {
  ...
  return transport->ops->resolve_device_path(transport->context, candidate_path,
                                             device_path, device_path_size,
                                             error, error_size);
}

bool sc_transport_send_hello(const ScTransport *transport,
                             const char *device_path, char *response,
                             size_t response_size, char *error,
                             size_t error_size) {
  if (transport == 0 || transport->ops == 0 ||
  ...
  return transport->ops->send_hello(transport->context, device_path, response,
                                    response_size, error, error_size);
}

bool sc_transport_send_sc_command(const ScTransport *transport,
                                  const char *device_path, const char *command,
                                  char *response, size_t response_size,
                                  char *error, size_t error_size) {
  if (transport == 0 || transport->ops == 0 ||
  ...
  return transport->ops->send_sc_command(transport->context, device_path,
                                         command, response, response_size,
                                         error, error_size);
}
