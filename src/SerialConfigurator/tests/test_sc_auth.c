 ...
 *
 * The salt, key derivation and challenge response come from JaszczurHAL
 * hal_sc_auth, the same code the firmware runs. These tests lock the salt
 * value and the message layout to byte-stable vectors, so a deliberate
 * change surfaces here rather than as "AUTH_FAILED" on the bench, and they
 * cover the host-only helpers from sc_auth.h.
 */
...
static int test_salt_is_fiesta_sc_auth_v1(void) {
  static const char k_expected_salt[] = "FIESTA-SC-AUTH-v1";
  TEST_ASSERT(HAL_SC_AUTH_SALT_LEN == sizeof(k_expected_salt) - 1u,
              "salt length must match \"FIESTA-SC-AUTH-v1\"");
  TEST_ASSERT(memcmp(HAL_SC_AUTH_SALT, k_expected_salt, HAL_SC_AUTH_SALT_LEN) ==
                  0,
...
static int test_key_derivation_is_hmac_sha256_of_uid(void) {
  ...
  const uint8_t uid[8] = {0xE6u, 0x61u, 0xA4u, 0xD1u,
                          0x23u, 0x45u, 0x67u, 0xABu};

  uint8_t key[HAL_SC_AUTH_KEY_BYTES];
  TEST_ASSERT(hal_sc_auth_derive_device_key(uid, sizeof(uid), key),
  ...
  TEST_ASSERT(sc_crypto_hmac_sha256(HAL_SC_AUTH_SALT, HAL_SC_AUTH_SALT_LEN, uid,
                                    sizeof(uid), expected),
              "reference hmac");
  ...
  uint8_t alt_key[HAL_SC_AUTH_KEY_BYTES];
  TEST_ASSERT(hal_sc_auth_derive_device_key(alt_uid, sizeof(alt_uid), alt_key),
              "derive alt key");
...
static int test_response_binds_challenge_and_session_id(void) {
  ...
  uint8_t resp_a[HAL_SC_AUTH_RESPONSE_BYTES];
  uint8_t resp_b[HAL_SC_AUTH_RESPONSE_BYTES];

  TEST_ASSERT(hal_sc_auth_compute_response(key, challenge, sizeof(challenge),
                                           0x12345678u, resp_a),
  ...
  uint8_t challenge2[HAL_SC_AUTH_CHALLENGE_BYTES];
  memcpy(challenge2, challenge, sizeof(challenge2));
  challenge2[0] ^= 0xFFu;
  uint8_t resp_c[HAL_SC_AUTH_RESPONSE_BYTES];
  TEST_ASSERT(hal_sc_auth_compute_response(key, challenge2, sizeof(challenge2),
                                           0x12345678u, resp_c),
...
static int test_response_matches_hand_computed_reference_vector(void) {
  ...
  char hex[HAL_SC_AUTH_RESPONSE_HEX_BUF_SIZE];
  TEST_ASSERT(sc_auth_compute_response_hex(uid, sizeof(uid), challenge,
                                           sizeof(challenge), session_id, hex,
                                           sizeof(hex)),
  ...
  uint8_t msg[HAL_SC_AUTH_CHALLENGE_BYTES + 4u];
  memcpy(msg, challenge, HAL_SC_AUTH_CHALLENGE_BYTES);
  msg[HAL_SC_AUTH_CHALLENGE_BYTES + 0u] = 0x12u;
  msg[HAL_SC_AUTH_CHALLENGE_BYTES + 1u] = 0x34u;
  msg[HAL_SC_AUTH_CHALLENGE_BYTES + 2u] = 0x56u;
  msg[HAL_SC_AUTH_CHALLENGE_BYTES + 3u] = 0x78u;

  uint8_t mac_ref[HAL_SC_AUTH_RESPONSE_BYTES];
  TEST_ASSERT(sc_crypto_hmac_sha256(key_ref, sizeof(key_ref), msg, sizeof(msg),
                                    mac_ref),
              "reference mac");

  char hex_ref[HAL_SC_AUTH_RESPONSE_HEX_BUF_SIZE];
  TEST_ASSERT(sc_crypto_sha256_hex /* hex helper */ != NULL, "linker sanity");
...
static int test_null_arg_safety(void) {
  uint8_t key[HAL_SC_AUTH_KEY_BYTES];
  TEST_ASSERT(!hal_sc_auth_derive_device_key(NULL, 0u, key),
              "NULL uid rejected");
  TEST_ASSERT(!hal_sc_auth_derive_device_key((const uint8_t *)"x", 1u, NULL),
              "NULL out rejected");

  uint8_t challenge[HAL_SC_AUTH_CHALLENGE_BYTES] = {0u};
  uint8_t resp[HAL_SC_AUTH_RESPONSE_BYTES];
  TEST_ASSERT(!hal_sc_auth_compute_response(NULL, challenge, sizeof(challenge),
                                            0u, resp),
              "NULL key rejected");
  TEST_ASSERT(
      !hal_sc_auth_compute_response(key, NULL, sizeof(challenge), 0u, resp),
      "NULL challenge rejected");
  TEST_ASSERT(!hal_sc_auth_compute_response(
                  key, challenge, HAL_SC_AUTH_CHALLENGE_BYTES + 1u, 0u, resp),
              "oversized challenge rejected");

  char hex[HAL_SC_AUTH_RESPONSE_HEX_BUF_SIZE];
  TEST_ASSERT(!sc_auth_compute_response_hex(
                  (const uint8_t *)"x", 1u, challenge, sizeof(challenge), 0u,
                  hex, HAL_SC_AUTH_RESPONSE_HEX_BUF_SIZE - 1u),
