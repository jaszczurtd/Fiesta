 ...
 *
 * Locks the JSON shape, hard-reject behaviour, and the rules for the
...
static int test_parse_minimal_valid_manifest(void) {
  sc_manifest_t m;
  const sc_manifest_status_t st = sc_manifest_parse(
      k_minimal_manifest_json, strlen(k_minimal_manifest_json), &m);
...
static int test_parse_with_signature(void) {
  const char *json =
      ...
      "}";
  sc_manifest_t m;
  TEST_ASSERT_EQ(SC_MANIFEST_OK, sc_manifest_parse(json, strlen(json), &m),
...
static int test_missing_required_field_is_rejected(void) {
  ...
  TEST_ASSERT_EQ(SC_MANIFEST_ERR_MISSING_FIELD,
                 sc_manifest_parse(json, strlen(json), &m), "missing build_id");
  return 0;
}

static int test_empty_object_is_rejected(void) {
  sc_manifest_t m;
  TEST_ASSERT_EQ(SC_MANIFEST_ERR_MISSING_FIELD, sc_manifest_parse("{}", 2u, &m),
                 "empty object");
  return 0;
}
...
static int test_unknown_field_is_rejected(void) {
  ...
  TEST_ASSERT_EQ(SC_MANIFEST_ERR_UNKNOWN_FIELD,
                 sc_manifest_parse(json, strlen(json), &m), "unknown field");
  return 0;
}
...
static int test_bad_sha256_format_is_rejected(void) {
  ...
  const char *json_short =
      ...
      "\"build_id\":\"x\","
      "\"sha256\":"
      "\"a7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad\""
      "}";
  sc_manifest_t m;
  ...
  /* Uppercase hex must be rejected; the manifest requires lowercase. */
...
static int test_load_file_round_trip(void) {
  ...
  TEST_ASSERT(write_tempfile(path, k_minimal_manifest_json,
  ...
  TEST_ASSERT(strcmp(m.module_name, SC_MODULE_TOKEN_ECU) == 0,
              "loaded module_name");
  return 0;
}
...
static int test_artifact_missing_returns_open_error(void) {
  ...
  TEST_ASSERT_EQ(SC_MANIFEST_OK,
                 ...
                 "parse");
  TEST_ASSERT_EQ(SC_MANIFEST_ERR_FILE_OPEN,
                 sc_manifest_verify_artifact(&m, "/nonexistent/artifact.uf2"),
...
static int test_resolve_uf2_path_from_manifest(void) {
  ...
  char out[512];
  TEST_ASSERT_EQ(SC_MANIFEST_OK,
                 sc_manifest_resolve_uf2_path("/tmp/fw/ecu.manifest.json", &m,
                                              out, sizeof(out)),
                 "resolve sidecar path");
  TEST_ASSERT(strcmp(out, "/tmp/fw/firmware.uf2") == 0, "resolved path");
  return 0;
}
...
static int test_status_strings_are_stable(void) {
  TEST_ASSERT(strcmp(sc_manifest_status_str(SC_MANIFEST_OK), "OK") == 0,
              "OK string");
  TEST_ASSERT(
      strcmp(sc_manifest_status_str(SC_MANIFEST_ERR_ARTIFACT_HASH_MISMATCH),
