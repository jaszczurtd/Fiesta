...
#define TEST_ASSERT_EQ(a, b, msg)                                              \
  do {                                                                         \
    if ((a) != (b)) {                                                          \
      fprintf(stderr, "FAIL: %s - %s (line %d): got %d, want %d\n", __func__,  \
              (msg), __LINE__, (int)(a), (int)(b));                            \
      return 1;                                                                \
    }                                                                          \
  } while (0)
...
static bool mock_send(void *ctx, const char *path, const char *cmd,
  ...
  if (strncmp(cmd, SC_CMD_SET_PARAM " ", sizeof(SC_CMD_SET_PARAM)) == 0) {
    if (!st->authenticated) {
    ...
    char id[64];
    int value = 0;
    if (!parse_set_param_args(cmd + sizeof(SC_CMD_SET_PARAM), id, sizeof(id),
                              &value)) {
      snprintf(response, response_size, "SC_BAD_REQUEST malformed");
      return true;
    }
    if (strcmp(id, MOCK_READ_ONLY_ID) == 0) {
      snprintf(response, response_size, "SC_BAD_REQUEST read_only id=%s", id);
      return true;
    }
    ...
    if (value < MOCK_WRITABLE_MIN || value > MOCK_WRITABLE_MAX) {
      snprintf(response, response_size,
               "SC_BAD_REQUEST out_of_range id=%s min=%d max=%d", id,
               MOCK_WRITABLE_MIN, MOCK_WRITABLE_MAX);
      return true;
    }
    st->staging_value = (int16_t)value;
    snprintf(response, response_size,
             "SC_OK PARAM_SET id=%s staged=%d active=%d", id, value,
             (int)st->active_value);
    return true;
  }
...
static int test_set_param_unknown_id_maps_to_invalid_id(void) {
  ...
  char err[512] = {0};
  const ScSetParamStatus rc =
      sc_core_set_param(&t, "/dev/mock", "no_such_param", 0, err, sizeof(err));
  TEST_ASSERT_EQ(SC_SET_PARAM_ERR_INVALID_ID, rc, "unknown id -> INVALID_ID");
...
static int test_set_param_read_only_maps_to_read_only(void) {
  ...
  TEST_ASSERT_EQ(SC_SET_PARAM_ERR_READ_ONLY, rc, "RO descriptor -> READ_ONLY");
...
static int test_set_param_out_of_range_carries_min_max(void) {
  ...
  TEST_ASSERT(strstr(err, "min=70") != NULL, "error buffer carries min");
  TEST_ASSERT(strstr(err, "max=130") != NULL, "error buffer carries max");
  return 0;
}
...
static int test_set_param_transport_failure_returns_transport(void) {
  ...
  const ScSetParamStatus rc = sc_core_set_param(
      &t, "/dev/mock", MOCK_WRITABLE_ID, 110, err, sizeof(err));
  TEST_ASSERT_EQ(SC_SET_PARAM_ERR_TRANSPORT, rc, "send failure -> TRANSPORT");
  return 0;
}
...
static int test_commit_params_happy_path(void) {
  ...
  TEST_ASSERT_EQ(SC_SET_PARAM_OK,
                 sc_core_set_param(&t, "/dev/mock", MOCK_WRITABLE_ID, 105, err,
                                   sizeof(err)),
                 "set precondition");

  err[0] = '\0';
  const ScCommitParamsStatus rc =
      sc_core_commit_params(&t, "/dev/mock", err, sizeof(err));
...
static int test_revert_params_happy_path(void) {
  ...
  char err[512] = {0};
  TEST_ASSERT_EQ(SC_SET_PARAM_OK,
                 sc_core_set_param(&t, "/dev/mock", MOCK_WRITABLE_ID, 120, err,
                                   sizeof(err)),
                 "set precondition");
  TEST_ASSERT_EQ(120, (int)st.staging_value, "staging shifted");

  err[0] = '\0';
  const ScRevertParamsStatus rc =
      sc_core_revert_params(&t, "/dev/mock", err, sizeof(err));
...
static int test_status_strings_are_stable(void) {
  TEST_ASSERT(strcmp(sc_set_param_status_name(SC_SET_PARAM_OK), "OK") == 0,
              "set OK");
  ...
  TEST_ASSERT(strcmp(sc_set_param_status_name(SC_SET_PARAM_ERR_READ_ONLY),
              ...
              "set RO");
  TEST_ASSERT(
      strcmp(sc_commit_params_status_name(SC_COMMIT_PARAMS_ERR_COMMIT_FAILED),
             "COMMIT_FAILED") == 0,
      "commit failed");
  TEST_ASSERT(strcmp(sc_revert_params_status_name(SC_REVERT_PARAMS_OK), "OK") ==
                  0,
              "revert OK");
  return 0;
}
