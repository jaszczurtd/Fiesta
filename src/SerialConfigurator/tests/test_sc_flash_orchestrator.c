 ...
 * response against the reference mock UID, SC_REBOOT_BOOTLOADER ACKs
...
static bool mock_send(void *ctx, const char *path, const char *cmd,
                      char *response, size_t response_size, char *err,
                      size_t err_size) {
  (void)path;
  (void)err;
  (void)err_size;
...
static void build_minimal_uf2(uint8_t out[512]) {
  ...
  out[16] = 0x00;
  out[17] = 0x01; /* payload_size = 256 LE */
...
static int fs_fixture_setup(FsFixture *fx, const char *manifest_module,
                            const char *manifest_fw,
                            const char *manifest_sha_override) {
  memset(fx, 0, sizeof(*fx));

  if (make_temp_dir("src", fx->src_dir, sizeof(fx->src_dir)) != 0)
    return -1;
  (void)snprintf(fx->src_uf2_path, sizeof(fx->src_uf2_path), "%s/firmware.uf2",
                 fx->src_dir);
  build_minimal_uf2(fx->uf2);
  if (write_file(fx->src_uf2_path, fx->uf2, sizeof(fx->uf2)) != 0)
    return -1;

  /* Compute the artifact's true sha256 so the happy-path manifest
   * matches it and the artifact-mismatch test can override. */
  if (!sc_crypto_sha256_hex(fx->uf2, sizeof(fx->uf2), fx->uf2_sha256_hex,
  ...
  if (manifest_module != NULL && manifest_fw != NULL) {
    ...
    const char *sha = (manifest_sha_override != NULL) ? manifest_sha_override
                                                      : fx->uf2_sha256_hex;
    char json[1024];
    ...
    if (n <= 0 || (size_t)n >= sizeof(json))
      return -1;
    if (write_file(fx->manifest_path, json, (size_t)n) != 0)
      return -1;
  }

  if (make_temp_dir("bs", fx->bootsel_parent, sizeof(fx->bootsel_parent)) != 0)
    return -1;
  (void)snprintf(fx->bootsel_drive, sizeof(fx->bootsel_drive), "%s/RPI-RP2",
                 fx->bootsel_parent);
  if (mkdir(fx->bootsel_drive, 0755) != 0)
    return -1;

  if (make_temp_dir("byid", fx->by_id_parent, sizeof(fx->by_id_parent)) != 0)
    return -1;
  ...
  if (write_file(fx->by_id_entry, "", 0u) != 0)
    return -1;
...
static int test_happy_path_with_manifest(void) {
  ...
  const ScFlashStatus rc = sc_core_flash(
      ...
      fx.src_uf2_path, fx.manifest_path, &opts, NULL, NULL, err, sizeof(err));
...
static int test_happy_path_no_manifest(void) {
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path, NULL,
                    &opts, NULL, NULL, NULL, 0u);
  TEST_ASSERT(rc == SC_FLASH_STATUS_OK, sc_flash_status_name(rc));
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_format_check_rejects_corrupt_uf2(void) {
  ...
  TEST_ASSERT(rc == SC_FLASH_STATUS_FORMAT_REJECTED, sc_flash_status_name(rc));
  TEST_ASSERT(st.hello_call_count == 0, "format check refuses BEFORE auth");
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_manifest_module_mismatch(void) {
  ...
  char err[512] = {0};
  const ScFlashStatus rc = sc_core_flash(
      &t, /*module_index=*/0, "/dev/mock", k_uid_hex, fx.src_uf2_path,
      fx.manifest_path, &opts, NULL, NULL, err, sizeof(err));
  TEST_ASSERT(rc == SC_FLASH_STATUS_MANIFEST_MODULE_MISMATCH,
              sc_flash_status_name(rc));
  TEST_ASSERT(st.hello_call_count == 0, "module mismatch refuses BEFORE auth");
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_manifest_artifact_mismatch(void) {
  ...
  TEST_ASSERT(fs_fixture_setup(&fx, SC_MODULE_TOKEN_ECU, "1.0.0", k_bad_sha) ==
                  0,
              "fixture");
  MockState st;
  mock_state_init(&st, "1.0.0");
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path,
                    fx.manifest_path, &opts, NULL, NULL, NULL, 0u);
...
static int test_auth_failure_propagates(void) {
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path, NULL,
                    &opts, NULL, NULL, NULL, 0u);
  TEST_ASSERT(rc == SC_FLASH_STATUS_AUTH_FAILED, sc_flash_status_name(rc));
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_reboot_failure_propagates(void) {
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path, NULL,
                    &opts, NULL, NULL, NULL, 0u);
  TEST_ASSERT(rc == SC_FLASH_STATUS_REBOOT_FAILED, sc_flash_status_name(rc));
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_bootsel_timeout_when_drive_never_appears(void) {
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path, NULL,
                    &opts, NULL, NULL, NULL, 0u);
  TEST_ASSERT(rc == SC_FLASH_STATUS_BOOTSEL_TIMEOUT, sc_flash_status_name(rc));
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_reenum_timeout_when_byid_entry_missing(void) {
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path, NULL,
                    &opts, NULL, NULL, NULL, 0u);
  TEST_ASSERT(rc == SC_FLASH_STATUS_REENUM_TIMEOUT, sc_flash_status_name(rc));
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_post_flash_fw_mismatch_with_manifest(void) {
  ...
  TEST_ASSERT(rc == SC_FLASH_STATUS_POST_FLASH_FW_MISMATCH,
              sc_flash_status_name(rc));
  TEST_ASSERT(strstr(err, "9.9.9") != NULL, "diagnostic mentions device fw");
  fs_fixture_teardown(&fx);
  return 0;
}
...
static void on_orchestrator_progress(ScFlashPhase phase, uint64_t bytes_written,
...
static int test_progress_callback_phases_flow(void) {
  ...
  const ScFlashStatus rc =
      sc_core_flash(&t, 0u, "/dev/mock", k_uid_hex, fx.src_uf2_path, NULL,
                    &opts, on_orchestrator_progress, &cap, NULL, 0u);
  TEST_ASSERT(rc == SC_FLASH_STATUS_OK, sc_flash_status_name(rc));
  ...
  TEST_ASSERT(cap.copy_last_total == 512u, "copy progress reports total bytes");
  fs_fixture_teardown(&fx);
  return 0;
}
...
static int test_status_and_phase_names_stable(void) {
  ...
  TEST_ASSERT(strcmp(sc_flash_status_name(SC_FLASH_STATUS_BOOTSEL_TIMEOUT),
                     "BOOTSEL_TIMEOUT") == 0,
              "BOOTSEL_TIMEOUT status");
  TEST_ASSERT(
      strcmp(sc_flash_status_name(SC_FLASH_STATUS_MANIFEST_MODULE_MISMATCH),
...
static int test_null_arg_rejection(void) {
  ...
  TEST_ASSERT(sc_core_flash(NULL, 0u, "/dev/x", "AB", "/tmp/x", NULL, NULL,
                            NULL, NULL, err,
                            sizeof(err)) == SC_FLASH_STATUS_NULL_ARG,
              "NULL transport");
  MockState st;
  mock_state_init(&st, "1.0.0");
  ScTransport t = make_transport(&st);
  TEST_ASSERT(sc_core_flash(&t, 0u, NULL, "AB", "/tmp/x", NULL, NULL, NULL,
                            NULL, err, sizeof(err)) == SC_FLASH_STATUS_NULL_ARG,
              "NULL device_path");
  TEST_ASSERT(sc_core_flash(&t, 0u, "/dev/x", NULL, "/tmp/x", NULL, NULL, NULL,
                            NULL, err, sizeof(err)) == SC_FLASH_STATUS_NULL_ARG,
              "NULL uid");
  TEST_ASSERT(sc_core_flash(&t, 0u, "/dev/x", "", "/tmp/x", NULL, NULL, NULL,
                            NULL, err, sizeof(err)) == SC_FLASH_STATUS_NULL_ARG,
              "empty uid");
  TEST_ASSERT(sc_core_flash(&t, 0u, "/dev/x", "AB", NULL, NULL, NULL, NULL,
                            NULL, err, sizeof(err)) == SC_FLASH_STATUS_NULL_ARG,
              "NULL uf2_path");
  TEST_ASSERT(sc_core_flash(&t, /*bad index=*/99u, "/dev/x", "AB", "/tmp/x",
