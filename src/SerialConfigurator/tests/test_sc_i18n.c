...
static int test_every_key_has_an_english_string(void) {
  ...
  for (int k = 0; k < SC_I18N_KEY_COUNT; ++k) {
    ...
    if (s == NULL || s[0] == '\0') {
      fprintf(stderr, "FAIL: %s - key %d returned %s\n", __func__, k,
              s == NULL ? "NULL" : "empty string");
      return 1;
    }
    if (strcmp(s, sc_i18n_missing_marker()) == 0) {
      fprintf(stderr, "FAIL: %s - key %d returned the missing marker\n",
              __func__, k);
      return 1;
    }
  }
...
static int test_every_key_has_a_polish_string(void) {
  ...
  for (int k = 0; k < SC_I18N_KEY_COUNT; ++k) {
    const char *s = sc_i18n_string_get((ScI18nKey)k);
    if (s == NULL || s[0] == '\0') {
      fprintf(stderr, "FAIL: %s - key %d returned %s under PL\n", __func__, k,
              s == NULL ? "NULL" : "empty string");
      return 1;
    }
    if (strcmp(s, sc_i18n_missing_marker()) == 0) {
...
static int test_locale_codes_are_iso639_two_letter(void) {
  ...
  volatile int invalid_locale = 42;
  /* NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_STR_EQ(sc_i18n_locale_code((ScLocale)invalid_locale), "en",
                     "OOR -> en");
  return 0;
}
...
static int test_parse_locale_from_string(void) {
  ...
  TEST_ASSERT(sc_i18n_parse_locale_string("pl-PL") == SC_LOCALE_PL,
              "pl-PL -> PL");
  /* Two-letter only */
  TEST_ASSERT(sc_i18n_parse_locale_string("pl") == SC_LOCALE_PL, "pl -> PL");
...
static int test_format_keys_round_trip_through_snprintf(void) {
  ...
  (void)snprintf(buf, sizeof(buf),
                 sc_i18n_string_get(SC_I18N_STATUS_CATALOG_READ_FMT), (size_t)5,
...
static int test_out_of_range_key_returns_marker(void) {
  ...
  volatile int invalid_neg = -1;
  volatile int invalid_off_end = SC_I18N_KEY_COUNT;
  /* NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_STR_EQ(sc_i18n_string_get((ScI18nKey)invalid_neg), marker,
                     "negative key");
  /* NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange) */
  TEST_ASSERT_STR_EQ(sc_i18n_string_get((ScI18nKey)invalid_off_end), marker,
                     "off-end key");
  return 0;
}
...
static int test_env_resolver_prefers_sc_locale_override(void) {
  ...
  unsetenv("LC_ALL");
  unsetenv("LC_MESSAGES");
  TEST_ASSERT(sc_i18n_active_locale() == SC_LOCALE_EN, "no env -> EN");
