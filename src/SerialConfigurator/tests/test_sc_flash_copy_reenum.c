...
static int make_src_file(const char *dir, size_t total, char *out_src,
                         size_t out_src_size) {
...
static int test_copy_writes_destination_byte_for_byte(void) {
  ...
  const size_t total = 200u * 1024u; /* 200 KiB -> 4 chunks of 64 KiB
                                        plus an 8 KiB tail. */
  TEST_ASSERT(make_src_file(src_dir, total, src_path, sizeof(src_path)) == 0,
              "make_src_file");

  progress_capture_t cap = {0};
  char err[256] = {0};
  const sc_flash_status_t st =
      sc_flash_copy_uf2(src_path, dst_dir, on_progress, &cap, err, sizeof(err));
  ...
  char dest_path[512];
  (void)snprintf(dest_path, sizeof(dest_path), "%s/firmware.uf2", dst_dir);
  uint8_t *expected_buf = (uint8_t *)malloc(total);
  uint8_t *got_buf = (uint8_t *)malloc(total);
  if (expected_buf == NULL || got_buf == NULL) {
    free(expected_buf);
    free(got_buf);
    TEST_ASSERT(false, "malloc");
  }
  for (size_t i = 0u; i < total; ++i) {
...
static int test_copy_progress_called_with_null_cb(void) {
  ...
  char src_path[512];
  TEST_ASSERT(make_src_file(src_dir, 1024u, src_path, sizeof(src_path)) == 0,
              "src");

  const sc_flash_status_t st =
      sc_flash_copy_uf2(src_path, dst_dir, NULL, NULL, NULL, 0u);
...
static int test_copy_rejects_missing_source(void) {
  ...
  char err[256] = {0};
  const sc_flash_status_t st =
      sc_flash_copy_uf2("/tmp/sc_flash_does_not_exist_xyz.uf2", dst_dir, NULL,
                        NULL, err, sizeof(err));
...
static int test_copy_rejects_unwritable_destination(void) {
  ...
  char err[256] = {0};
  const sc_flash_status_t st =
      sc_flash_copy_uf2(src_path, "/tmp/sc_flash_dst_does_not_exist_qq", NULL,
                        NULL, err, sizeof(err));
  TEST_ASSERT(st == SC_FLASH_ERR_FILE_WRITE, "missing destination rejected");
...
static int test_copy_rejects_empty_source(void) {
  ...
  char err[256] = {0};
  const sc_flash_status_t st =
      sc_flash_copy_uf2(src_path, dst_dir, NULL, NULL, err, sizeof(err));
...
static int test_copy_rejects_null_paths(void) {
  char err[64] = {0};
  TEST_ASSERT(sc_flash_copy_uf2(NULL, "/tmp", NULL, NULL, err, sizeof(err)) ==
                  SC_FLASH_ERR_NULL_ARG,
              "NULL src rejected");
  TEST_ASSERT(sc_flash_copy_uf2("/tmp/x", NULL, NULL, NULL, err, sizeof(err)) ==
                  SC_FLASH_ERR_NULL_ARG,
              "NULL drive rejected");
  return 0;
}
...
static int test_reenum_returns_path_when_uid_appears(void) {
  ...
  (void)snprintf(ca.path, sizeof(ca.path), "%s/usb-Jaszczur_Fiesta_ECU_%s-if00",
                 parent, uid);
  ca.delay_ms = 200u;
...
static int test_reenum_picks_uid_among_many_entries(void) {
  ...
  for (size_t i = 0u; i < 3u; ++i) {
    (void)snprintf(extras[i], sizeof(extras[i]),
                   "%s/usb-Other_Device_OTHER_%s-if00", parent, uids[i]);
  ...
  (void)snprintf(target, sizeof(target),
                 "%s/usb-Jaszczur_Fiesta_Clocks_E661A4D1234567AB-if00", parent);
...
static int test_reenum_rejects_null_inputs(void) {
  char err[64] = {0};
  TEST_ASSERT(sc_flash__wait_reenumeration_in(NULL, "FOO", 100u, NULL, 0u, err,
                                              sizeof(err)) ==
                  SC_FLASH_ERR_NULL_ARG,
              "NULL parent rejected");
  TEST_ASSERT(sc_flash__wait_reenumeration_in("/tmp", NULL, 100u, NULL, 0u, err,
                                              sizeof(err)) ==
                  SC_FLASH_ERR_NULL_ARG,
              "NULL uid rejected");
  TEST_ASSERT(sc_flash__wait_reenumeration_in("/tmp", "", 100u, NULL, 0u, err,
                                              sizeof(err)) ==
                  SC_FLASH_ERR_NULL_ARG,
              "empty uid rejected");
  return 0;
...
static int test_status_strings_for_new_codes(void) {
  TEST_ASSERT(
      strcmp(sc_flash_status_str(SC_FLASH_ERR_FILE_WRITE), "FILE_WRITE") == 0,
