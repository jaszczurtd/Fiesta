/*
 * Known-answer tests for the sc_crypto entry points. They run against the
 * JaszczurHAL hal_crypto sources compiled into serial_configurator_core.
 */
...
static int test_md5_contract(void) {
  ...
  TEST_ASSERT(sc_crypto_md5(input, sizeof(input), digest),
              "MD5 should succeed");
  TEST_ASSERT(memcmp(digest, expected_digest, sizeof(digest)) == 0,
              "MD5 digest mismatch");
  TEST_ASSERT(sc_crypto_md5_hex(input, sizeof(input), hex, sizeof(hex)),
              "MD5 hex should succeed");
  TEST_ASSERT(strcmp(hex, "900150983cd24fb0d6963f7d28e17f72") == 0,
              "MD5 hex mismatch");
  return 0;
}
...
static int test_base64_contract(void) {
  ...
  /* The line protocol carries base64-encoded build_id and similar fields. */
  TEST_ASSERT(sc_crypto_base64_encoded_len(sizeof(raw)) == 8u,
              "encoded len mismatch");
  TEST_ASSERT(sc_crypto_base64_decoded_max_len(8u) == 6u,
              "decoded max len mismatch");
  TEST_ASSERT(sc_crypto_base64_encode(raw, sizeof(raw), encoded,
                                      sizeof(encoded), &encoded_len),
              "base64 encode should succeed");
  TEST_ASSERT(encoded_len == 8u, "encoded length mismatch");
  TEST_ASSERT(strcmp(encoded, "Rmllc3Rh") == 0,
              "base64 encoded value mismatch");

  TEST_ASSERT(sc_crypto_base64_decode(encoded, encoded_len, decoded,
                                      sizeof(decoded), &decoded_len),
              "base64 decode should succeed");
  TEST_ASSERT(decoded_len == sizeof(raw), "decoded length mismatch");
  TEST_ASSERT(memcmp(decoded, raw, sizeof(raw)) == 0,
              "base64 decoded bytes mismatch");
  return 0;
}
...
static int test_chacha20_roundtrip(void) {
  ...
  TEST_ASSERT(sc_crypto_chacha20_xor(key, 1u, nonce, plaintext,
                                     sizeof(plaintext), ciphertext),
              "chacha20 xor encrypt failed");
  TEST_ASSERT(sc_crypto_chacha20_xor(key, 1u, nonce, ciphertext,
                                     sizeof(ciphertext), decrypted),
              "chacha20 xor decrypt failed");
  TEST_ASSERT(memcmp(decrypted, plaintext, sizeof(plaintext)) == 0,
              "chacha20 roundtrip mismatch");

  TEST_ASSERT(sc_crypto_chacha20_poly1305_encrypt(key, nonce, 0, 0u, plaintext,
                                                  sizeof(plaintext), ciphertext,
                                                  tag),
              "chacha20-poly1305 encrypt failed");
  TEST_ASSERT(sc_crypto_chacha20_poly1305_decrypt(key, nonce, 0, 0u, ciphertext,
                                                  sizeof(ciphertext), tag,
                                                  decrypted),
              "chacha20-poly1305 decrypt failed");
  TEST_ASSERT(memcmp(decrypted, plaintext, sizeof(plaintext)) == 0,
              "chacha20-poly1305 roundtrip mismatch");

  memcpy(tampered_tag, tag, sizeof(tag));
  tampered_tag[0] ^= 0x01u;
  TEST_ASSERT(!sc_crypto_chacha20_poly1305_decrypt(
                  key, nonce, 0, 0u, ciphertext, sizeof(ciphertext),
...
static int test_sha256_vectors(void) {
  ...
  TEST_ASSERT(sc_crypto_sha256((const uint8_t *)"abc", 3u, digest),
              "sha256(abc) failed");
  TEST_ASSERT(memcmp(digest, k_sha256_abc, sizeof(digest)) == 0,
              "sha256(abc) mismatch");
  TEST_ASSERT(
      sc_crypto_sha256_hex((const uint8_t *)"abc", 3u, hex, sizeof(hex)),
      "sha256_hex(abc) failed");
  TEST_ASSERT(
      strcmp(
          hex,
          "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad") ==
          0,
      "sha256_hex(abc) mismatch");

  TEST_ASSERT(sc_crypto_sha256(NULL, 0u, digest), "sha256(empty) failed");
  TEST_ASSERT(memcmp(digest, k_sha256_empty, sizeof(digest)) == 0,
  ...
  TEST_ASSERT(
      sc_crypto_sha256_hex(long_input, sizeof(long_input), hex, sizeof(hex)),
      "sha256(long) failed");
  /* Result is just expected to be deterministic; primarily exercises
   * multi-block compress. */
...
static int test_hmac_sha256_vectors(void) {
  ...
  const uint8_t key1[20] = {0x0bu, 0x0bu, 0x0bu, 0x0bu, 0x0bu, 0x0bu, 0x0bu,
  ...
  TEST_ASSERT(
      sc_crypto_hmac_sha256(key1, sizeof(key1), data1, sizeof(data1), mac),
      "hmac case1 failed");
  TEST_ASSERT(memcmp(mac, expected1, sizeof(mac)) == 0, "hmac case1 mismatch");
  TEST_ASSERT(sc_crypto_hmac_sha256_hex(key1, sizeof(key1), data1,
                                        sizeof(data1), hex, sizeof(hex)),
              "hmac_hex case1 failed");
  TEST_ASSERT(
      strcmp(
          hex,
          "b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7") ==
          0,
      "hmac_hex case1 mismatch");

  /* RFC 4231 Test Case 2: key="Jefe", data="what do ya want for nothing?". */
  const uint8_t key2[] = {'J', 'e', 'f', 'e'};
  const uint8_t data2[] = "what do ya want for nothing?";
  TEST_ASSERT(sc_crypto_hmac_sha256_hex(key2, sizeof(key2), data2,
                                        sizeof(data2) - 1u, hex, sizeof(hex)),
              "hmac case2 failed");
  TEST_ASSERT(
      strcmp(
          hex,
  ...
  const uint8_t key4[25] = {0x01u, 0x02u, 0x03u, 0x04u, 0x05u, 0x06u, 0x07u,
                            0x08u, 0x09u, 0x0au, 0x0bu, 0x0cu, 0x0du, 0x0eu,
                            0x0fu, 0x10u, 0x11u, 0x12u, 0x13u, 0x14u, 0x15u,
                            0x16u, 0x17u, 0x18u, 0x19u};
  uint8_t data4[50];
  memset(data4, 0xcdu, sizeof(data4));
  TEST_ASSERT(sc_crypto_hmac_sha256_hex(key4, sizeof(key4), data4,
                                        sizeof(data4), hex, sizeof(hex)),
              "hmac case4 failed");
  TEST_ASSERT(
      strcmp(
          hex,
  ...
  TEST_ASSERT(sc_crypto_hmac_sha256_hex(key6, sizeof(key6), data6,
                                        sizeof(data6) - 1u, hex, sizeof(hex)),
              "hmac case6 failed");
  TEST_ASSERT(
      strcmp(
          hex,
  ...
  /* Reject NULL output. */
  TEST_ASSERT(
      !sc_crypto_hmac_sha256(key1, sizeof(key1), data1, sizeof(data1), NULL),
      "hmac NULL out should fail");
  return 0;
}
