 ...
 *   - blob_encode for the reference ECU V2 layout matches
...
static const sc_param_descriptor_t k_ecu_descs[] = {
    ...
    SC_PARAM_SCALAR_I16("fan_coolant_stop_c", ecu_values_t, fan_coolant_stop_c,
                        50, 120, 85, 1, "cooling_fan"),
    SC_PARAM_SCALAR_I16("fan_air_start_c", ecu_values_t, fan_air_start_c, 20,
                        90, 60, 1, "cabin_fan"),
    SC_PARAM_SCALAR_I16("fan_air_stop_c", ecu_values_t, fan_air_stop_c, -20, 80,
                        50, 1, "cabin_fan"),
    SC_PARAM_SCALAR_I16("heater_stop_c", ecu_values_t, heater_stop_c, 40, 100,
                        75, 1, "engine_heater"),
    SC_PARAM_SCALAR_I16("nominal_rpm", ecu_values_t, nominal_rpm, 700, 1200,
                        850, 2, "idle"),
};
static const size_t k_ecu_descs_count = COUNTOF(k_ecu_descs);
...
static const size_t k_clocks_descs_count = COUNTOF(k_clocks_descs);
...
static void capture_reset(capture_t *cap) { memset(cap, 0, sizeof(*cap)); }
...
static int test_find_by_id_paths(void) {
  ...
  TEST_ASSERT(sc_param_find_by_id(k_ecu_descs, k_ecu_descs_count, NULL) == NULL,
              "find_by_id NULL id returns NULL");
  TEST_ASSERT(sc_param_find_by_id(k_ecu_descs, k_ecu_descs_count, "missing") ==
                  NULL,
...
static int test_get_set_i16_guards(void) {
  ...
  /* Successful set. */
  TEST_ASSERT(sc_param_set_i16(&k_ecu_descs[0], &v, 100), "valid set succeeds");
  ...
  TEST_ASSERT(!sc_param_set_i16(&k_clocks_descs[0], &c, 95), "RO set rejected");
  return 0;
}
...
static int test_reply_param_values(void) {
  ...
  TEST_ASSERT(strcmp(cap.lines[0], "SC_OK PARAM_VALUES fan_coolant_start_c=95"
...
static int test_reply_get_param_emits_general_for_empty_group(void) {
  ...
  capture_t cap;
  capture_reset(&cap);
  sc_param_reply_get_param(k_solo_descs, 1u, &v, "solo", capture_emit, &cap);
  TEST_ASSERT(cap.count == 1u, "one line emitted");
...
static int test_reply_get_param_happy_and_invalid(void) {
  ...
  capture_t cap;
  capture_reset(&cap);
  sc_param_reply_get_param(k_ecu_descs, k_ecu_descs_count, &v, "heater_stop_c",
                           capture_emit, &cap);
...
static int test_blob_size_for_schema(void) {
  TEST_ASSERT(
      sc_param_blob_size_for_schema(k_ecu_descs, k_ecu_descs_count, 1) == 16u,
      "ECU V1 blob size matches legacy");
  TEST_ASSERT(
      sc_param_blob_size_for_schema(k_ecu_descs, k_ecu_descs_count, 2) == 18u,
...
static int test_blob_round_trip_v2(void) {
  ...
  uint8_t blob[18];
  const size_t written = sc_param_blob_encode(k_ecu_descs, k_ecu_descs_count,
                                              &in, 2, blob, sizeof(blob));
...
static int test_blob_decode_rejects_bad_inputs(void) {
  ...
  uint8_t tiny[3] = {0};
  TEST_ASSERT(!sc_param_blob_decode(k_ecu_descs, k_ecu_descs_count, &out, tiny,
                                    sizeof(tiny), NULL),
...
static int test_crc32_reference_vector(void) {
  ...
  TEST_ASSERT(sc_param_crc32(buf, 9u) == 0xCBF43926u, "CRC32 reference vector");
...
static int test_reply_set_param_writes_only_staging(void) {
  ...
  const bool ok = sc_param_reply_set_param(
      k_ecu_descs, k_ecu_descs_count, &staging, &active, "fan_coolant_start_c",
      110, capture_emit, &cap);
  TEST_ASSERT(ok, "set_param happy path returns true");
  TEST_ASSERT(cap.count == 1u, "one reply line");
  TEST_ASSERT(strcmp(cap.lines[0], "SC_OK PARAM_SET id=fan_coolant_start_c"
  ...
  TEST_ASSERT(staging.heater_stop_c == 75, "unrelated staging field untouched");
  return 0;
}
...
static int test_reply_set_param_unknown_id_rejected(void) {
  ...
  capture_reset(&cap);
  const bool ok =
      sc_param_reply_set_param(k_ecu_descs, k_ecu_descs_count, &staging,
                               &active, "nope", 0, capture_emit, &cap);
...
static int test_reply_set_param_read_only_rejected(void) {
  ...
  capture_t cap;
  capture_reset(&cap);
  const bool ok = sc_param_reply_set_param(k_clocks_descs, k_clocks_descs_count,
                                           &staging, &active, "coolant_warn_c",
                                           100, capture_emit, &cap);
  TEST_ASSERT(!ok, "RO descriptor returns false");
  TEST_ASSERT(cap.count == 1u, "one reply line");
  TEST_ASSERT(
      strcmp(cap.lines[0], "SC_BAD_REQUEST read_only id=coolant_warn_c") == 0,
      "RO reply format");
...
static int test_reply_set_param_out_of_range_rejected(void) {
  ...
  TEST_ASSERT(strcmp(cap.lines[0],
              ...
              "OOR reply format");
  TEST_ASSERT(staging.fan_coolant_start_c == 95, "staging untouched on OOR");
  return 0;
}
...
static int test_copy_active_to_staging_round_trip(void) {
  ...
  ecu_values_t active = {
      .fan_coolant_start_c = 100,
      .fan_coolant_stop_c = 80,
      .fan_air_start_c = 55,
      .fan_air_stop_c = 30,
      .heater_stop_c = 60,
      .nominal_rpm = 1100,
...
static int test_copy_staging_to_active_round_trip(void) {
  ecu_values_t staging = {
      .fan_coolant_start_c = 105,
      .fan_coolant_stop_c = 90,
      .fan_air_start_c = 70,
      .fan_air_stop_c = 40,
      .heater_stop_c = 65,
      .nominal_rpm = 950,
...
static int test_copy_null_inputs_safe(void) {
  ...
  TEST_ASSERT(
      sc_param_copy_active_to_staging(NULL, k_ecu_descs_count, &v, &v) == 0u,
  ...
  TEST_ASSERT(
      sc_param_copy_staging_to_active(NULL, k_ecu_descs_count, &v, &v) == 0u,
