 ...
 *  on AVR ATmega328P-class boards
 *
...
#if defined I2C
  TWI_Start();
  TWI_address(SSD1306_ADDRESS, 0);
  TWI_Write(0x00); // 0x00 for command, 0x40 for data
...
void lcd_data(uint8_t data[], uint16_t size) {
  ...
  TWI_Start();
  TWI_address(SSD1306_ADDRESS, 0);
  TWI_Write(0x40); // 0x00 for command, 0x40 for data
...
#elif defined SH1106
  uint8_t commandSequence[] = {0xb0 + y, 0x21, 0x00 + ((2 + x) & (0x0f)),
                               0x10 + (((2 + x) & (0xf0)) >> 4), 0x7f};
#endif
  lcd_command(commandSequence, sizeof(commandSequence));
}
...
void lcd_putc(char c) {
  ...
  default:
    // char doesn't fit in line
    {
      ...
      if (((int)cursorPosition.x >= max_char_x) || (c < ' '))
        break;
    }
    // mapping char
    c -= ' ';
    ...
    if (charMode == DOUBLESIZE) {
      ...
      for (uint8_t i = 0; i < sizeof(FONT[0]); i++) {
        ...
        displayBuffer[cursorPosition.y + 1][cursorPosition.x + (2 * i)] =
            doubleChar[i] >> 8;
        displayBuffer[cursorPosition.y + 1][cursorPosition.x + (2 * i) + 1] =
            doubleChar[i] >> 8;
        displayBuffer[cursorPosition.y][cursorPosition.x + (2 * i)] =
            doubleChar[i] & 0xff;
        displayBuffer[cursorPosition.y][cursorPosition.x + (2 * i) + 1] =
            doubleChar[i] & 0xff;
      }
...
#if defined(SSD1306) || defined(SSD1309)
      uint8_t commandSequence[] = {0xb0 + cursorPosition.y + 1, 0x21,
                                   cursorPosition.x, 0x7f};
#elif defined SH1106
      uint8_t commandSequence[] = {
          0xb0 + cursorPosition.y + 1, 0x21,
          0x00 + ((2 + cursorPosition.x) & (0x0f)),
          0x10 + (((2 + cursorPosition.x) & (0xf0)) >> 4), 0x7f};
#endif
      lcd_command(commandSequence, sizeof(commandSequence));
...
#elif defined SH1106
      commandSequence[2] =
          0x00 + ((2 + cursorPosition.x + (2 * sizeof(FONT[0]))) & (0x0f));
      commandSequence[3] =
          0x10 +
          (((2 + cursorPosition.x + (2 * sizeof(FONT[0]))) & (0xf0)) >> 4);
#endif
      lcd_command(commandSequence, sizeof(commandSequence));
      cursorPosition.x += sizeof(FONT[0]) * 2;
    } else {
...
void lcd_drawRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2,
                  uint8_t color) {
  if (px1 > DISPLAY_WIDTH - 1 || px2 > DISPLAY_WIDTH - 1 ||
      py1 > DISPLAY_HEIGHT - 1 || py2 > DISPLAY_HEIGHT - 1)
    return;
...
void lcd_drawBitmap(uint8_t x, uint8_t y, const uint8_t *picture, uint8_t width,
