...
void TempGauge::drawTempBar(int x, int y, int currentHeight, int color) {
  ...
  if (currentHeight > 0) {
    ...
    hal_display_fill_rect(x, y - currentHeight, TEMP_BAR_WIDTH, currentHeight,
                          color);
  }

  int remainingHeight = TEMP_BAR_MAXHEIGHT - currentHeight;
  if (remainingHeight > 0) {
    hal_display_fill_rect(x, y - TEMP_BAR_MAXHEIGHT, TEMP_BAR_WIDTH,
                          remainingHeight, ICONS_BG_COLOR);
  }
}

void TempGauge::redraw(void) { drawOnce = true; }
...
int TempGauge::currentValToHeight(int currentVal, int maxVal) {
  ...
  return hal_math_percent_to_value(percent, TEMP_BAR_MAXHEIGHT);
}
...
void TempGauge::showTemperatureAmount(int currentVal) {
  ...
  if (drawOnce) {
    ...
    hal_display_draw_image(getBaseX(), getBaseY(), BIG_ICONS_WIDTH,
                           BIG_ICONS_HEIGHT, ICONS_BG_COLOR, tempImg);
    drawOnce = false;
  } else {
    ...
    int currentHeight = currentValToHeight(
        (currentVal < temp_max) ? currentVal : temp_max, maxVal);
    ...
    if (draw) {
      ...
      case TEMP_GAUGE_COOLANT:
        if (fanEnabled && img != NULL) {
          ...
          hal_display_draw_rgb_bitmap(x, y, img, FAN_COOLANT_WIDTH,
                                      FAN_COOLANT_HEIGHT);
        } else {
