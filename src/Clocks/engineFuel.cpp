...
void redrawFuel(void) { fuel_g.redraw(); }

void drawFuelEmpty(void) { fuel_g.drawFuelEmpty(); }

void showFuelAmount(void) {
  fuel_g.showFuelAmount((int)valueFields[F_FUEL], FUEL_MIN - FUEL_MAX);
}

void initFuelMeasurement(void) { fuel_g.init(); }
...
void EngineFuelGauge::init() {
  ...
  nextMeasurement = hal_get_seconds() + fuelMeasurementTime;
  measurements = 0;

  hal_display_set_default_font();
  hal_display_get_text_bounds(emptyMessage, &emptyMessageWidth,
                              &emptyMessageHeight);
...
void EngineFuelGauge::redraw(void) { f_drawOnce = true; }

int EngineFuelGauge::getBaseX(void) { return (OFFSET * 2) + FUEL_WIDTH + 4; }

int EngineFuelGauge::getBaseY(void) {
  return SCREEN_H - FUEL_HEIGHT - 8 - (OFFSET * 2);
}

int EngineFuelGauge::getWidth(void) { return FUEL_GAUGE_WIDTH - getBaseX(); }
...
void EngineFuelGauge::showFuelAmount(int currentVal, int maxVal) {
  ...
  currentFuelWidth = hal_math_percent_to_value(percent, width);
  ...
  if (f_drawOnce) {
    ...
    hal_display_fill_rect(FUEL_WIDTH + OFFSET + (OFFSET / 2), y,
                          getWidth() + OFFSET, SCREEN_H - y, ICONS_BG_COLOR);

    x = getBaseX();

    hal_display_draw_image(x - FUEL_WIDTH - OFFSET, y, FUEL_WIDTH, FUEL_HEIGHT,
                           0, (uint16_t *)fuelIcon);
    ...
    drawChangeableFuelContent(currentFuelWidth, FUEL_GAUGE_HEIGHT,
                              getGaugePos());
  }
  drawFuelEmpty();
}
...
void EngineFuelGauge::drawChangeableFuelContent(int w, int fh, int y) {
  ...
  int width = getWidth();
  int minW = hal_math_percent_to_value(MINIMUM_FUEL_AMOUNT_PERCENTAGE, width);
  if (w <= minW && w >= 1) {
    draw = true;
    if (alertSwitch()) {
      color = HAL_COLOR(RED);
