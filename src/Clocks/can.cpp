...
static void onCanFrame(uint32_t canID, uint8_t len, const uint8_t *buf) {
  ...
  case CAN_ID_ECU_UPDATE_01: {
    ...
    valueFields[F_OIL_TEMP] =
        static_cast<int8_t>(buf[CAN_FRAME_ECU_UPDATE_OIL]);
  } break;
  ...
  case CAN_ID_ECU_UPDATE_03: {
    ...
    valueFields[F_PRESSURE_PERCENTAGE] =
        buf[CAN_FRAME_ECU_UPDATE_PRESSURE_PERCENTAGE];
    valueFields[F_FUEL_TEMP] =
        static_cast<int8_t>(buf[CAN_FRAME_ECU_UPDATE_FUEL_TEMP]);
...
bool isOilSpeedModuleConnected(void) { return oilSpeedModuleConnected; }

void canCheckConnection(void) {
  oilSpeedModuleConnected =
      (oilSpeedModuleMessages != lastOilSpeedModuleMessages);
...
int getEngineRPM(void) { return int(valueFields[F_RPM]); }

bool isEngineRunning(void) { return (getEngineRPM() != 0); }

bool isFanEnabled(void) { return valueFields[F_FAN_ENABLED] > 0; }

bool isDPFRegenerating(void) { return valueFields[F_DPF_REGEN] > 0; }

bool isDPFConnected(void) { return dpfConnected; }

float readFuel(void) { return valueFields[F_FUEL]; }

int getCurrentCarSpeed(void) { return int(valueFields[F_ABS_CAR_SPEED]); }

int getGPSSpeed(void) { return int(valueFields[F_GPS_CAR_SPEED]); }

float getOilPressure(void) { return valueFields[F_OIL_PRESSURE]; }

bool isGPSAvailable(void) { return valueFields[F_GPS_IS_AVAILABLE] > 0; }
