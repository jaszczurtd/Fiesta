...
void PressureGauge::drawTextForPressureIndicators(int x, int y,
                                                  const char *format, ...) {
...
void PressureGauge::redraw(void) { drawOnce = true; }
...
void PressureGauge::showPressureGauge(void) {
  ...
  if (drawOnce) {
    ...
    fiesta_can_split_decimal_tenths(current, &hi, &lo);
    ...
    case PRESSURE_G_TURBO:
      if (current > TURBO_MIN_PRESSURE_FOR_SPINNING) {
        ...
        if (draw && img != NULL) {
          ...
          hal_display_draw_rgb_bitmap(x, y, img, PRESSURE_ICONS_WIDTH,
                                      PRESSURE_ICONS_HEIGHT);
        }
      }
      break;
    }
  ...
  case PRESSURE_G_TURBO: {
    ...
    if (hi != lastHI_d || lo != lastLO_d) {
      ...
      w = hal_display_prepare_text(txt, DISPLAY_TXT_SIZE, "req:%d.%dBAR", hi,
                                   lo);
