...
void test_adjustometer_ema_uses_minimum_positive_step(void) {
  TEST_ASSERT_EQUAL_UINT32(
      1001U, adj_test_sensors_apply_adjustometer_ema(1001U, 1000U));
}

void test_adjustometer_ema_uses_minimum_negative_step(void) {
  TEST_ASSERT_EQUAL_UINT32(
      999U, adj_test_sensors_apply_adjustometer_ema(999U, 1000U));
}

void test_adc_ema_initializes_from_negative_previous(void) {
  TEST_ASSERT_FLOAT_WITHIN(0.001f, 12.5f,
                           adj_test_sensors_apply_adc_ema(12.5f, -1.0f));
}
...
void test_internal_state_round_trip(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(expected.baselineStableWindows,
                           actual.baselineStableWindows);
  ...
  TEST_ASSERT_EQUAL_UINT8(expected.zeroCandidateWindows,
                          actual.zeroCandidateWindows);
  TEST_ASSERT_FLOAT_WITHIN(0.001f, expected.filteredFuelTemp,
                           actual.filteredFuelTemp);
  TEST_ASSERT_FLOAT_WITHIN(0.001f, expected.filteredVoltage,
                           actual.filteredVoltage);
}
...
void test_baseline_convergence_enters_verification_after_last_stable_window(
    void) {
  const uint32_t periodUs = 100U;
  const uint32_t nowUs = (ADJUSTOMETER_BASELINE_MIN_TIME_MS * 1000U) +
                         (kPulseWindow * periodUs) + 1000U;
...
void test_baseline_verification_drift_restarts_convergence(void) {
  ...
  TEST_ASSERT_FALSE(actual.verifying);
  TEST_ASSERT_FALSE(actual.baselineReady);
  TEST_ASSERT_EQUAL_UINT32(12500U, actual.filteredHz);
  TEST_ASSERT_EQUAL_UINT32(12500U, actual.signalHz);
  TEST_ASSERT_EQUAL_UINT32(12500U, actual.baselineEstimate);
...
void test_baseline_verification_success_sets_ready_and_resets_zero_hold(void) {
  const uint32_t periodUs = 100U;
  const uint32_t nowUs = (ADJUSTOMETER_BASELINE_VERIFY_MS * 1000U) +
                         (kPulseWindow * periodUs) + 1000U;
...
void test_zero_hold_requires_two_same_sign_windows_to_release(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(1250U, afterFirst.filteredHz);
  ...
  TEST_ASSERT_EQUAL_UINT32(1437U, afterSecond.filteredHz);
  TEST_ASSERT_EQUAL_INT32(437, afterSecond.pulse);
}
...
void test_zero_hold_sign_change_restarts_candidate_sequence(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(830U, actual.filteredHz);
  TEST_ASSERT_EQUAL_INT32(0, actual.pulse);
}
...
int main(void) {
  ...
  RUN_TEST(
      test_baseline_convergence_enters_verification_after_last_stable_window);
